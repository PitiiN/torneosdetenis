...
import { clearStoredOrgSelection, getStoredOrgSelection, OrgSelection, setStoredOrgSelection } from "./src/lib/orgSelection";
...
function tabIconForRoute(name: string, focused: boolean): string {
  if (name === "Home") return focused ? "🏠" : "🏡";
  if (name === "Torneos") return focused ? "🏆" : "🎾";
  if (name === "Ranking") return focused ? "📈" : "📊";
  if (name === "Solicitudes") return focused ? "💳" : "🧾";
  if (name === "Notificaciones") return focused ? "🔔" : "🔕";
  return focused ? "⚙️" : "🛠️";
}
      ...
      <PlayerTab.Navigator
        screenOptions={({ route }) => ({
          ...
          tabBarIcon: ({ color, size, focused }) => (
            <Text style={{ fontSize: Math.max(size - 1, 14), color }}>{tabIconForRoute(route.name, focused)}</Text>
          ),
          sceneStyle: { backgroundColor: t.colors.deepNavy },
          headerTitleAlign: "center",
        })}
      >
        <PlayerTab.Screen name="Home" component={HomeScreen} />
...
function AdminTabs({ context }: { context: ProfileContext }) {
  const t = useTheme();
  const insets = useSafeAreaInsets();
  return (
    <NavigationContainer>
      ...
      >
        ...
        <AdminTab.Screen name="Config">
          {() => <AdminConfigScreen orgId={context.orgId} />}
        </AdminTab.Screen>
      </AdminTab.Navigator>
    </NavigationContainer>
  );
    ...
    try {
      ...
      if (res.data.session?.user) {
        ...
        if (!isAdminLike(ctx.role)) {
          const stored = await getStoredOrgSelection();
