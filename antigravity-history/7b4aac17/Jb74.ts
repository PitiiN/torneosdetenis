    ...
    try {
        ...
        if (updatedBooking) {
            ...
            if (updateData.status === 'PAGADA' && safeExisting.status !== 'PAGADA') {
                ...
                log('[Email Debug] Status changed to PAGADA. Attempting to send email.');
                ...
                if (!userEmail && safeBooking.user_id) {
                    ...
                    log(`[Email Debug] User email fetched manually: ${userEmail}`);
                }
                ...
                if (userEmail && safeBooking.field?.name) {
                    ...
                    try {
                        ...
                        log(`[Email Debug] Failed to send confirmation email: ${errorMsg}`);
                        console.error('[Email Debug] Failed to send confirmation email', e)
                    }
                } else {
                    log('[Email Debug] Missing email or field name. Skipping email.');
                }
            } else {
                // Also log when condition fails to see why
                const fs = await import('fs');
                const path = await import('path');
                const logFile = path.resolve(process.cwd(), 'debug-email.log');
                const log = (msg: string) => {
                ...
                log(`[Email Debug] Condition not met. UpdateStatus: ${updateData.status}, ExistingStatus: ${safeExisting.status}`);
            }
        }
