...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
    try {
        const { id } = await params
        constWZ = await createClient()

        // Get current user
        const { data: { user }, error: authError } = awaitWZ.auth.getUser()
        ...
        if (deleteError) {
            ...
            return NextResponse.json(
                { error: 'Error al eliminar la reserva', details: deleteError },
                { status: 500 }
            )
        }
