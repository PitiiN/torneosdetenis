...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
    try {
        ...
        let updatedBooking = { ...updatedBookingRaw }
        ...
        if (updatedBooking) {
            ...
            if (updateData.status === 'PAGADA' && existingBooking.status !== 'PAGADA') {
                ...
                if (!userEmail && updatedBooking.user_id) {
                    const { data: userData } = await adminClient.from('profiles').select('email').eq('id', updatedBooking.user_id).single()
                    userEmail = userData?.email
                }

                if (userEmail && updatedBooking.field?.name) {
                    ...
                    try {
                        ...
                        await sendBookingConfirmationEmail(userEmail, updatedBooking.field.name)
                    } catch (e) {
