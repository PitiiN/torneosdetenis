...
### Financial Panel Enhancements (Completed)
...
- Support `month` and `fieldId` filters.
#### [MODIFY] [route.ts](file:///src/app/api/admin/blocks/route.ts)
   - Fixed table name reference from `field_blocks` to `blocks`.

### Admin Tabs Fixes
#### [NEW] [AdminCalendarView.tsx](file:///src/components/admin/AdminCalendarView.tsx)
#### [NEW] [tabs.tsx](file:///src/components/ui/tabs.tsx)
#### [MODIFY] [page.tsx](file:///src/app/(dashboard)/admin/availability/page.tsx)
#### [MODIFY] [page.tsx](file:///src/app/(dashboard)/admin/bookings/page.tsx)
   - Refactored calendar logic into reusable component.
   - Added "Calendario" tab to Bookings page.

### Blocks Enhancement
#### [MODIFY] [route.ts](file:///src/app/api/admin/blocks/route.ts)
   - Update GET to include bookings with status 'BLOQUEADA'.
#### [MODIFY] [route.ts](file:///src/app/api/admin/blocks/[id]/route.ts)
   - Update DELETE to handle deletion of bookings with status 'BLOQUEADA'.

### Debtors & User Detail Enhancements
#### [MODIFY] [debtors/route.ts](file:///c:/Users/Asus/OneDrive%20-%20CORPORACIÓN%20XIX%20JUEGOS%20PANAMERICANOS%20SANTIAGO%202023/Escritorio/PitiN/Antigravity/ArriendoCanchas/src/app/api/admin/debtors/route.ts)
- Add `fieldId` query parameter support.
- Filter unpaid bookings by `fieldId` if provided.

#### [MODIFY] [bookings/route.ts](file:///c:/Users/Asus/OneDrive%20-%20CORPORACIÓN%20XIX%20JUEGOS%20PANAMERICANOS%20SANTIAGO%202023/Escritorio/PitiN/Antigravity/ArriendoCanchas/src/app/api/admin/bookings/route.ts)
- Add `userId` query parameter to allow fetching specific user history.

#### [MODIFY] [financial/page.tsx](file:///c:/Users/Asus/OneDrive%20-%20CORPORACIÓN%20XIX%20JUEGOS%20PANAMERICANOS%20SANTIAGO%202023/Escritorio/PitiN/Antigravity/ArriendoCanchas/src/app/(dashboard)/admin/financial/page.tsx)
- Only show "Usuarios con Deuda" list when a specific `fieldId` is selected.
- Pass `fieldId` to debtors API.

#### [MODIFY] [BulkPaymentModal.tsx](file:///c:/Users/Asus/OneDrive%20-%20CORPORACIÓN%20XIX%20JUEGOS%20PANAMERICANOS%20SANTIAGO%202023/Escritorio/PitiN/Antigravity/ArriendoCanchas/src/components/admin/BulkPaymentModal.tsx)
- Fetch full booking history (paid + unpaid) for the user and selected field on mount.
- Display mixed list of bookings with clear status indicators.
- Allow selecting only unpaid bookings for bulk payment.
...
- [ ] **Date Consistency**:
    - [ ] Update `api/admin/field-users` to accept `month` parameter and filter bookings by date (matching summary stats).
    - [ ] Update `api/admin/field-users` to include anonymous bookings (user_id null) grouped as "Usuario Anónimo" to match counts.
    - [ ] Update `admin/financial/page.tsx` to pass `month` to `field-users` fetch.

- [ ] **Modal Enhancements**:
    - [x] Update `BulkPaymentModal` to accept `month` and filter displayed bookings.
    - [x] Implement "Edit Price" feature in `BulkPaymentModal` (pencil icon -> input -> save).
    - [x] Create/Update API to handle price updates.
    - [ ] **Fix Price Edit**: Ensure input is accessible and saves correctly (review z-index/click prop).
...
- [ ] **UI/UX Polish**
    - [ ] **Fluid Transitions**: Optimize Financial Panel loading state.
        - [ ] Switch from full-page spinner to opacity/overlay for field filters.
        - [ ] Keep full loading for date changes (or optimize both if better).

## Verification
- [ ] Check Tabancura 6 (10 bookings) -> Should show "Usuario Anónimo" or users with bookings in Feb.
- [ ] Check Huelén 5 -> User detail should only show Feb bookings, not March.
- [ ] Verify Count Consistency (Summary vs List). estado
