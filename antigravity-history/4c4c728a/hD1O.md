...
### 3. Roles en Español (Mapeo de UI)
...
> [!IMPORTANT]
> Los valores técnicos en la base de datos se mantendrán como `student`, `coach`, `admin` para no romper las políticas de seguridad (RLS) ni la lógica de navegación. Solo se cambiará cómo se muestran y seleccionan en la interfaz.

- Cambiar las etiquetas de los roles en el picker de asignación y en los listados:
  - `student` -> `Alumno`
  - `coach` -> `Profesor`
  - `admin` -> `Admin`
- Se implementará un mapeo simple: `{ student: 'Alumno', coach: 'Profesor', admin: 'Admin' }`.
