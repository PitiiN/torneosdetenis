...
export default function AdminCreateClassScreen() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // 1. Fetch potentially overlapping classes on this court
            // We use the full range from first class start to last class end
            ...
            if (conflictingDates.length > 0) {
                ...
                if (finalClassesToInsert.length === 0) {
                    useAlertStore.getState().showAlert(
                        'Todo en Conflicto ⚠️',
                        `Ya existen clases en la ${selectedCourt.name} para todos los días seleccionados: ${conflictDetails}.`
                    );
                    setSubmitting(false);
                    return;
                }
                ...
                useAlertStore.getState().showAlert(
                    ...
                    `Ya existen clases en la ${selectedCourt.name} para los siguientes días: ${conflictDetails}.\n\n¿Deseas crear solo las clases en los días disponibles?`,
                    [
                    ...
                    <View style={styles.modalContent}>
                        ...
                        {selectedCoach && (
                            <TouchableOpacity
                                ...
                                onPress={() => { setSelectedCoach(null); setShowCoachModal(false); }}
                            >
                                <Ionicons name="person-remove" size={20} color={colors.error} />
                                <Text style={styles.removeCoachText}>Quitar profesor asignado</Text>
                            </TouchableOpacity>
                        )}
                        <FlatList
...
const styles = StyleSheet.create({
    ...
    removeCoachText: { fontSize: 14, fontWeight: '600', color: colors.error },
});
