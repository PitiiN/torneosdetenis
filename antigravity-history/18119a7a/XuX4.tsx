...
export default function AdminCreateClassScreen() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Use Chile offset (-04:00 / -03:00) to ensure UTC-stored times are caught
            const firstDate = classesToInsert[0].start_datetime.split('T')[0];
            const lastDate = classesToInsert[classesToInsert.length - 1].end_datetime.split('T')[0];
            const { data: existing } = await supabase
                .from('classes')
                .select('start_datetime, end_datetime')
                .eq('court_id', mainCourt.id)
                .gte('start_datetime', firstDate + 'T00:00:00-04:00')
                .lte('end_datetime', lastDate + 'T23:59:59-03:00');
