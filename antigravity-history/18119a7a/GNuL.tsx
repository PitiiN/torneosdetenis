...
export default function AdminCreateClassScreen() {
    ...
    const handleSubmit = async () => {
        if (!title.trim()) { Alert.alert('Error', 'Ingresa un nombre para la clase'); return; }
        if (selectedHour === null) { Alert.alert('Error', 'Selecciona un bloque horario'); return; }
        if (!selectedCourt) { Alert.alert('Error', 'Selecciona una cancha'); return; }
        if (!selectedCategory) { Alert.alert('Error', 'Selecciona una categoría'); return; }
        ...
        if (error) {
            ...
            Alert.alert('✅ Clase creada', `"${title}" fue creada exitosamente.`, [
