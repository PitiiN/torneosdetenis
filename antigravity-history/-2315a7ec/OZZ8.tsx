...
import {
    ...
    RefreshControl, Modal, ActivityIndicator, Alert, Switch, TextInput,
} from 'react-native';
...
export default function HomeScreen() {
    ...
    const loadAllowance = useCallback(async () => {
        if (!profile) return;
        const { data } = await supabase.rpc('get_student_class_summary', {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                // Perform essential loads
                await Promise.all([loadClasses(), loadAllowance()]);
            } catch (error) {
                console.error('Initialization error:', error);
            } finally {
                // Always set to loaded to avoid infinite spinner, even if error occurred
                setIsLoaded(true);
            }
        };
        init();
    ...
    }, [profile, loadClasses, loadAllowance]);
    ...
    const handleEnroll = async (cls: any) => {
        ...
        try {
            if (recurringEnabled) {
                ...
                const { availableClasses, conflicts, remainingCredits } =
                    await enrollmentsService.validateRecurringEnrollment(cls.id, profile.id, count);

                if (availableClasses.length === 0) {
                    const conflictMsg = conflicts.map(c => `- ${c.date}: ${c.reason}`).join('\n');
                    useAlertStore.getState().showAlert('Sin clases disponibles', `Todas las fechas seleccionadas tienen conflictos:\n\n${conflictMsg}`);
                    setEnrolling(false);
                    return;
                }

                if (!isAdmin && remainingCredits < availableClasses.length) {
                    useAlertStore.getState().showAlert(
                        'Créditos insuficientes',
                        `Necesitas ${availableClasses.length} clases para estas fechas, pero solo tienes ${remainingCredits} disponibles.`
                    );
                    setEnrolling(false);
                    return;
                }
                ...
                const performEnrollment = async (classes: any[]) => {
                    await enrollmentsService.enrollBatch(classes.map(c => c.id), profile.id);
                    useAlertStore.getState().showAlert('✅ Éxito', `Inscripción completada para ${classes.length} clases.`);
                ...
                if (conflicts.length > 0) {
                    ...
                    useAlertStore.getState().showAlert(
                        ...
                        `No pudimos incluir todas las clases:\n\n${conflictMsg}\n\n¿Deseas inscribirte solo en las ${availableClasses.length} clases disponibles?`,
                        [
                            { text: 'Cancelar', style: 'cancel', onPress: () => setEnrolling(false) },
                    ...
                    <View style={styles.modalContent}>
                        {selectedClass && (
                            <>
                                ...
                                ) : new Date(selectedClass.start_datetime) < new Date() ? (
                                    <View style={[styles.fullBanner, { backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1 }]}>
                                        <Text style={[styles.fullBannerText, { color: colors.textTertiary }]}>Clase finalizada</Text>
                                    </View>
                                ) : selectedClass.available_spots > 0 ? (
                                    <View style={styles.enrollContainer}>
                                        <View style={styles.recurringBlock}>
                                            <View style={styles.recurringHeader}>
                                                <View style={styles.recurringTextContent}>
                                                    <Text style={styles.recurringTitle}>Replicar inscripción</Text>
                                                    <Text style={styles.recurringSubtitle}>Repetir esta clase las próximas semanas</Text>
                                                </View>
                                                <Switch
                                                    value={recurringEnabled}
                                                    onValueChange={setRecurringEnabled}
                                                    trackColor={{ false: colors.surface, true: colors.primary[600] }}
                                                    thumbColor={recurringEnabled ? colors.primary[400] : colors.textTertiary}
                                                />
                                            </View>

                                            {recurringEnabled && (
                                                <View style={styles.recurringInputRow}>
                                                    <Text style={styles.recurringInputLabel}>Semanas adicionales:</Text>
                                                    <View style={styles.replicationInputWrapper}>
                                                        <TextInput
                                                            style={styles.replicationInput}
                                                            value={replicationCount}
                                                            onChangeText={(val) => setReplicationCount(val.replace(/[^0-9]/g, ''))}
                                            ...
                                            {enrolling ? (
                                                ...
                                                <>
                                                    ...
                                                    <Text style={styles.enrollButtonText}>
                                                        {recurringEnabled ? `Confirmar ${1 + (parseInt(replicationCount) || 1)} Inscripciones` : 'Confirmar Inscripción'}
                                                    </Text>
                                                </>
                                            )}
                                        </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    // Recurring Enrollment UI
    enrollContainer: { gap: spacing.lg, paddingVertical: spacing.sm },
    recurringBlock: {
    ...
    recurringTextContent: { flex: 1, gap: 2 },
    recurringTitle: { fontSize: 15, fontWeight: '600', color: colors.text },
    recurringSubtitle: { fontSize: 12, color: colors.textTertiary },
    recurringInputRow: {
        flexDirection: 'row',
        alignItems: 'center',
        justifyContent: 'space-between',
    ...
    recurringInputLabel: { fontSize: 14, color: colors.textSecondary },
    replicationInputWrapper: {
