...
export default function HomeScreen() {
    ...
    const handleCancelEnrollment = async (cls: any) => {
        ...
        if (hoursUntilClass < 8) {
            useAlertStore.getState().showAlert('No se puede cancelar', 'Solo puedes cancelar hasta 8 horas antes del inicio de la clase.');
            return;
        }
                                    ...
                                    >
                                        ...
                                        {hasClass && (
                                            <>
                                                ...
                                                <Text style={[
                                                    styles.hourStatus,
                                                    isCancelled ? { color: colors.textTertiary } : (isFull ? styles.hourStatusFull : styles.hourStatusAvailable)
                                                ]}>
                                                    {isCancelled ? 'Bloqueado' : (isFull ? 'Lleno' : `${cls.available_spots} cupos`)}
                                                </Text>
                                            </>
                                        )}
                                    </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    hourBlockCancelled: { backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, opacity: 0.8 },
