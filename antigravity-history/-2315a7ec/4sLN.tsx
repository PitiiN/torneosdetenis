...
export default function HomeScreen() {
    ...
    const handleEnroll = async (cls: any) => {
        ...
        // Validation
        const count = recurringEnabled ? parseInt(replicationCount) || 1 : 0;
        const totalNeeded = 1 + count;

        if (allowance && allowance.remaining_classes < totalNeeded && !isAdmin) {
            useAlertStore.getState().showAlert(
                'Sin créditos suficientes',
                `Necesitas ${totalNeeded} clases para esta inscripción y sus repeticiones, pero solo tienes ${allowance.remaining_classes} disponibles.`
            );
            return;
        }
        ...
        try {
            if (recurringEnabled) {
                await enrollmentsService.enrollRecurring(cls.id, profile.id, count);
                useAlertStore.getState().showAlert('✅ Inscripciones Exitosas', `Te inscribiste en "${cls.title}" por ${totalNeeded} semanas.`);
            } else {
