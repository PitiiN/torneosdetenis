...
export default function HomeScreen() {
    ...
    const handleEnroll = async (cls: any) => {
        ...
        if (allowance && allowance.remaining_classes <= 0 && !isAdmin) {
            Alert.alert('Sin cupos', 'Has alcanzado el límite de clases de tu plan. Revisa tus pagos para ampliar.');
            return;
        }
        ...
        try {
            ...
            Alert.alert('✅ Inscrito', `Te inscribiste en "${cls.title}"`);
    ...
    const handleCancelEnrollment = async (cls: any) => {
        ...
        // Check 8-hour rule
        const classTime = new Date(cls.start_datetime).getTime();
        const now = Date.now();
                    ...
                    <View key={block.label} style={styles.blockSection}>
                        ...
                        <View style={styles.hoursGrid}>
                            {Array.from({ length: block.end - block.start }, (_, i) => {
                                ...
                                const hasClass = !!cls;
                                const isFull = hasClass && (cls.available_spots <= 0);
                                const isAvailable = hasClass && cls.available_spots > 0;
                                ...
                                return (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            styles.hourBlock,
                                            hasClass && (isFull ? styles.hourBlockFull : styles.hourBlockAvailable),
                                            !hasClass && styles.hourBlockEmpty,
                                        ]}
                                        disabled={!hasClass}
                                        onPress={() => hasClass && openClassModal(cls)}
                                        activeOpacity={0.7}
                                    >
                                        <Text style={[styles.hourText, hasClass && styles.hourTextActive]}>
                                            {String(hour).padStart(2, '0')}:00
                                        </Text>
                                        {hasClass && (
                                            <Text style={[styles.hourStatus, isFull ? styles.hourStatusFull : styles.hourStatusAvailable]}>
...
const styles = StyleSheet.create({
    ...
    hourStatus: { fontSize: 10, fontWeight: '600', marginTop: 2 },
