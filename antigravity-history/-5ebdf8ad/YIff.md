...
- [ ] Actualizar schema `Announcement` y `store.ts` para soportar media en respuestas.
- [ ] Implementar botón de colapso de hilos ("Ver X comentarios").
- [ ] Diseñar UI para input multimedia (icono clip 📎) con validación 10MB/50MB simulada.
- [ ] Actualizar pantalla de Admin para crear Avisos (lugar, fecha).

## 2. Módulo: Encuestas (Smart-Polling)
- [ ] Definir schema `Poll` en `store.ts` con opciones, votos y deadline.
- [ ] Crear componente interaccional UI para feed de Encuestas (barras animadas proporcionales).
- [ ] Implementar lógica de validación (encuesta expirada = deshabilitado).
- [ ] Formularios de creación (Admin) con toggle de notificación.
