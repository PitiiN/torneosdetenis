                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingVertical: spacing.md }}>
                        <View style={{ flexDirection: 'row', gap: ROUND_GAP }}>
                            {rounds.map((roundNum, roundIndex) => {
                                const roundMatches = filteredMatches.filter(m => m.round_number === roundNum).sort((a, b) => a.match_order - b.match_order);
                                const numMatchesInRound = roundMatches.length;
                                const prevRoundNumMatches = roundIndex > 0 ? filteredMatches.filter(m => m.round_number === rounds[roundIndex - 1]).length : 0;
