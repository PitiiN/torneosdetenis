                ...
                {filteredMatches.length === 0 ? (
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingVertical: spacing.md }}>
                        <View style={{ flexDirection: 'row', gap: spacing['3xl'], paddingHorizontal: spacing.xl }}>
                            {(() => {
                                const activeFilteredMatches = matches.filter(m => {
                                    if (activeTab === 'consolacion') return m.round_name.startsWith('Consolación');
                                    return !m.round_name.startsWith('Consolación');
                                });
                                const activeRounds = [...new Set(activeFilteredMatches.map(m => m.round_number))].sort((a, b) => a - b);

                                return activeRounds.map((roundNum, index) => {
                                    const roundMatches = activeFilteredMatches.filter(m => m.round_number === roundNum);
                                        ...
                                        <View key={roundNum} style={{ width: 260 }}>
                                            ...
                                            <View style={{ marginTop: initialMarginTop }}>
                                                {roundMatches.map((m: any, mIdx: number) => {
                                                    ...
                                                    // Use same logic as admin for 3rd place positioning
                                                    const nextIs3rdPlace = mIdx < roundMatches.length - 1 && roundMatches[mIdx + 1]?.round_name.includes('3er y 4to');
                                                    const currentMatchGap = nextIs3rdPlace ? spacing.xl : matchGap;

                                                    // Split score robustly
                                                    const scores = m.score ? m.score.split(/[\s-]+/) : ['-', '-'];
                                                    ...
                                                    return (
                                                        <View
                                                            ...
                                                            style={[
                                                                ...
                                                                {
                                                                    height: MATCH_HEIGHT,
                                                                    marginBottom: mIdx === roundMatches.length - 1 ? 0 : currentMatchGap
                                                                }
                                                            ]}
                                                        >
                                                            {m.round_name.includes('3er y 4to Puesto') && (
                                                            ...
                                                            <View style={{ padding: spacing.md, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.background, flex: 1 }}>
                                                                ...
                                                                <View style={{ backgroundColor: isUnplayed ? colors.background : colors.primary[500], paddingHorizontal: 8, paddingVertical: 2, borderRadius: borderRadius.sm, minWidth: 32, alignItems: 'center' }}>
                                                                    <Text style={{ color: isUnplayed ? colors.textSecondary : '#fff', fontSize: 12, fontWeight: '700' }}>{isUnplayed ? '-' : scores[0]}</Text>
                                                                </View>
                                                            </View>
                                                            <View style={{ padding: spacing.md, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', opacity: isTBD ? 0.6 : 1, flex: 1 }}>
                                                                ...
                                                                <View style={{ backgroundColor: isUnplayed ? colors.background : colors.primary[500], paddingHorizontal: 8, paddingVertical: 2, borderRadius: borderRadius.sm, minWidth: 32, alignItems: 'center' }}>
                                                                    <Text style={{ color: isUnplayed ? colors.textSecondary : '#fff', fontSize: 12, fontWeight: '700' }}>{isUnplayed ? '-' : scores[1] || '0'}</Text>
                                                                </View>
                                                            </View>
                                                            {isTBD && (
