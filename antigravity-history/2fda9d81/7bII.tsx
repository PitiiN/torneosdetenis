...
export default function AdminTournamentDetailScreen() {
    ...
    const saveMatchScore = async () => {
        ...
        setSavingMatch(true);
        try {
            // Placeholder: determine winner based on input? Or just save text.
            // A real system would have pickers for winner.
            const { error } = await supabase
                .from('tournament_matches')
                .update({ score: scoreInput })
                .eq('id', selectedMatch.id);

            if (error) throw error;

            // Update local state early
            setMatches(matches.map(m => m.id === selectedMatch.id ? { ...m, score: scoreInput } : m));
                ...
                {matches.length === 0 ? (
                    ...
                    <View style={{ gap: spacing.xl }}>
                        ...
                        <View style={{ backgroundColor: colors.surface, borderRadius: borderRadius.xl, borderWidth: 1, borderColor: colors.border, overflow: 'hidden' }}>
                            ...
                            {/* Dummy data based on selected players for this group */}
                            {players.map((p, idx) => (
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingVertical: spacing.md }}>
                        <View style={{ flexDirection: 'row', minWidth: 800, gap: spacing['3xl'] }}>
                            {[...new Set(matches.map(m => m.round_number))].sort().map(roundNum => {
                                ...
                                return (
                                    <View key={roundNum} style={{ flex: 1, minWidth: 260, justifyContent: 'space-around', gap: spacing.xl }}>
                                        <Text style={{ color: colors.textTertiary, fontSize: 12, fontWeight: '700', textTransform: 'uppercase', marginBottom: spacing.md, letterSpacing: 1, paddingHorizontal: spacing.sm }}>{roundMatches[0]?.round_name}</Text>
                                        ...
                                        <View style={{ gap: spacing.xl }}>
                                            {roundMatches.map(m => {
                                                ...
                                                return (
                                                    <View
                                                        key={m.id}
                                                        style={{ backgroundColor: colors.surface, borderRadius: borderRadius.lg, borderWidth: 1, borderColor: colors.border, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 1 }}
                                                    >
                                                        ...
                                                        <TouchableOpacity
                                                            style={{ padding: spacing.md, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: colors.background }}
                                                        ...
                                                        >
                                                            ...
                                                            <TouchableOpacity onPress={() => handleMatchPress(m)} style={{ backgroundColor: isUnplayed ? colors.background : colors.primary[500], paddingHorizontal: 8, paddingVertical: 2, borderRadius: borderRadius.sm }}>
                                                                <Text style={{ color: isUnplayed ? colors.textSecondary : '#fff', fontSize: 12, fontWeight: '700' }}>{isUnplayed ? '-' : m.score.split(' ')[0]}</Text>
                                                            </TouchableOpacity>
                                                        </TouchableOpacity>

                                                        <TouchableOpacity
                                                            style={{ padding: spacing.md, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', opacity: isTBD ? 0.6 : 1 }}
