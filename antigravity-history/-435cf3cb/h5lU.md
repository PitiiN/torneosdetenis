...
1. Create `.env.production` with safe public keys:
   ...
   NEXT_PUBLIC_APP_URL=...
   ```
2. **Crucial**: Ensure `.env.production` is NOT in `.gitignore` or `.dockerignore` for the production build. Git will silently ignore the file if it's in `.gitignore`, even if you try to `git add` it (unless you use `-f`).
   - **Symptom of missing file**: Even after creating `.env.production`, the 500 error persists because the file is not on GitHub or the deployment server.
   - **Fix**: Remove `.env.production` from `.gitignore`.
