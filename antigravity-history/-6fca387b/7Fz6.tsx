...
import {
    ...
    TextInput, ActivityIndicator, Modal, FlatList, Switch,
} from 'react-native';
...
export default function AdminEditClassScreen() {
    ...
    const [selectedCourt2Category, setSelectedCourt2Category] = useState<any>(null);
    ...
    const loadClass = useCallback(async () => {
        ...
        *, courts (name), 
        ...
        court2_cat:class_categories!classes_court2_category_id_fkey (id, name, color)
      `)
            .eq('id', id)
            .single();
        if (data) {
            ...
            setSelectedCourt2Category(data.court2_cat);
        }
    }, [id]);

    const loadData = useCallback(async () => {
        const [catsRes, courtsRes] = await Promise.all([
    ...
    const handleSave = async () => {
        ...
        const { error } = await supabase
            ...
            .update({
                ...
                court2_category_id: selectedCourt2Category?.id || cls?.court2_category_id,
            })
            .eq('id', id);
        setSaving(false);
        if (error) {
            ...
            <ScrollView contentContainerStyle={styles.form}>
                ...
                <View style={styles.courtCatBlock}>
                    {courts.slice(0, 2).map((court, idx) => {
                        const selected = idx === 0 ? selectedCategory : selectedCourt2Category;
                        const setter = idx === 0 ? setSelectedCategory : setSelectedCourt2Category;
                        return (
                    ...
                    <View style={styles.modalContent}>
                        ...
                        {studentSearch.length > 0 ? (
                            ...
                            <View style={{ paddingVertical: spacing.xl, alignItems: 'center' }}>
                                <Ionicons name="search" size={40} color={colors.textTertiary} style={{ marginBottom: spacing.sm }} />
                                <Text style={styles.noResults}>Escribe para buscar un alumno</Text>
                            </View>
                        )}
                    </View>
                </View>
