...
export function AdminScreen() {
  ...
  function openScheduleModal(match: Match) {
    setScheduleTargetMatch(match);
    setScheduleModalVisible(true);
  }

  async function saveMatchSchedule(payload: { courtId: string; startAt: string; status: "SCHEDULED" | "IN_PLAY" | "DELAYED" | "DONE" }) {
    if (!scheduleTargetMatch) return;

    const existing = schedules.find((s) => s.match_id === scheduleTargetMatch.id);
    if (existing) {
      const u = await supabase
        .from("schedules")
        .update({
      ...
      const i = await supabase.from("schedules").insert({
        match_id: scheduleTargetMatch.id,
        court_id: payload.courtId,
        start_at: payload.startAt,
        status: payload.status,
      });
    ...
    setScheduleTargetMatch(null);
    Alert.alert("Listo", "Programación guardada.");
    loadData();
  }
                      ...
                      {drawMatches.map(m => (
                        <View key={m.id} style={styles.matchCard}>
                          ...
                          {schedulesByMatch.get(m.id) && (
                            <Text style={{ color: '#00FFFF', fontSize: 13, marginTop: 4 }}>
                              🗓 {courtNameById.get(schedulesByMatch.get(m.id)?.court_id ?? "")} | {schedulesByMatch.get(m.id)?.start_at} ({schedulesByMatch.get(m.id)?.status})
                            </Text>
                          )}
                          ...
                          <View style={{ marginTop: 8, flexDirection: "row", gap: 8 }}>
                            ...
                            <View style={{ flex: 1 }}>
                              <Button color="#B8E600" title="Programar" onPress={() => openScheduleModal(m)} />
                            </View>
                          </View>
                        </View>
                      ))}
