...
export function AdminScreen() {
  ...
  async function assignPlayerToSlot(userId: string) {
    ...
    if (assignTarget.mode === "MATCH") {
      const u = await supabase.from("matches").update({ [`player${assignTarget.playerNum}_id`]: cleanUserId }).eq("id", assignTarget.matchId);
      if (u.error) Alert.alert("Error", u.error.message);
    } else if (assignTarget.mode === "GROUP") {
      const u = await supabase.from("rr_group_members").update({ user_id: cleanUserId }).eq("id", assignTarget.memberId);
      if (u.error) Alert.alert("Error", u.error.message);
    }
          ...
          <>
            ...
            <View style={styles.block}>
              ...
              {categories.filter(c => c.tournament_id === selectedTournamentId).map((c) => {
                ...
                if (isRoundRobin) {
                  hasPlayers = groups.some(g => (membersByGroup.get(g.id) ?? []).some(m => m.user_id != null));
                } else {
                  hasPlayers = drawMatches.some(m => m.player1_id || m.player2_id);
                }
                              ...
                              {members.map(m => (
                                <Text key={m.id} style={styles.text}>
                                  ...
                                  <Text style={{ color: '#B8E600' }} onPress={() => openGroupAssignModal(m.id, c.id)}>
                                    {` ${profileNameById.get(m.user_id) ?? "Vacío"}`}
                                  </Text>
                                </Text>
                              ))}
                            </View>
