...
export function AdminScreen() {
  ...
  async function assignPlayerToSlot(userId: string) {
    if (!assignTarget) return;
    if (assignTarget.mode === "MATCH") {
      const u = await supabase.from("matches").update({ [`player${assignTarget.playerNum}_id`]: userId }).eq("id", assignTarget.matchId);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.block}>
          ...
          {categories.map((c) => {
            ...
            const groups = groupsByDraw.get(draw.id) ?? [];
            const hasPlayers = isRoundRobin
              ? groups.some(g => (membersByGroup.get(g.id) ?? []).length > 0)
              : drawMatches.some(m => m.player1_id || m.player2_id);
            ...
            return (
              <View key={c.id} style={styles.card}>
                ...
                {!hasPlayers && (
                  <Button color="#B8E600" title="Sortear Jugadores" onPress={() => runSorteo(c.id)} />
                )}

                {isRoundRobin ? (
                  <View style={{ marginTop: 8 }}>
                    <Button color="#0B00FF" title="Cerrar fase RR y crear Llave" onPress={() => closeRrPhase(c.id)} />
                    {buildStandingsForCategory(c.id).map(({ group, standings }) => (
                      <View key={group.id} style={styles.card}>
                        ...
                        {standings.length === 0 && <Text style={styles.text}>Sin jugadores asignados</Text>}
                      </View>
                    ))}
                  </View>
                ) : null}
                    ...
                    <View key={m.id} style={styles.matchCard}>
                      <Text style={styles.text}>
                        ...
                        <Text style={{ color: '#B8E600' }} onPress={() => openMatchAssignModal(m, 1)}>
                          {` ${profileNameById.get(m.player1_id ?? "") ?? "Vacío"} `}
                        </Text>
                        vs
                        <Text style={{ color: '#B8E600' }} onPress={() => openMatchAssignModal(m, 2)}>
                          {` ${profileNameById.get(m.player2_id ?? "") ?? "Vacío"} `}
                        </Text>
                        | {m.status}
                      </Text>
                      <View style={{ marginTop: 4 }}>
                        <Button color="#0B00FF" title="Editar Score" onPress={() => openMatchEditor(m)} />
                      </View>
                    </View>
                  ))}
                </View>
