...
export function AdminScreen() {
  ...
  async function assignPlayerToSlot({ userId, manualName }: { userId: string | null; manualName: string | null }) {
    ...
    if (assignTarget.mode === "MATCH") {
      ...
      const u = await supabase.from("matches").update({ [field]: userId, [nameField]: manualName }).eq("id", assignTarget.matchId);
      if (u.error) Alert.alert("Error", u.error.message);
    } else if (assignTarget.mode === "GROUP") {
      const u = await supabase.from("rr_group_members").update({ user_id: userId, manual_name: manualName }).eq("id", assignTarget.memberId);
      if (u.error) Alert.alert("Error", u.error.message);
    }
                              ...
                              {members.map(m => (
                                <Text key={m.id} style={styles.text}>
                                  ...
                                  <Text style={{ color: '#B8E600' }} onPress={() => openGroupAssignModal(m.id, c.id)}>
                                    {` ${m.user_id ? profileNameById.get(m.user_id) : m.manual_name || "Vacío"}`}
                                  </Text>
                                </Text>
                              ))}
                            </View>
                        ...
                        <View key={m.id} style={styles.matchCard}>
                          <Text style={styles.text}>
                            ...
                            <Text style={{ color: '#B8E600' }} onPress={() => openMatchAssignModal(m, 1)}>
                              {` ${m.player1_id ? profileNameById.get(m.player1_id) : m.player1_manual_name || "Vacío"} `}
                            </Text>
                            vs
                            <Text style={{ color: '#B8E600' }} onPress={() => openMatchAssignModal(m, 2)}>
                              {` ${m.player2_id ? profileNameById.get(m.player2_id) : m.player2_manual_name || "Vacío"} `}
                            </Text>
                            | {m.status}
                          </Text>
