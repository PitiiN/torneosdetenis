...
export default function AdminFinanceScreen() {
    ...
    const load = useCallback(async () => {
        const { data } = await supabase
            ...
            .select('*, profiles!payment_receipts_student_id_fkey (id, full_name, email)')
    ...
    const filteredReceipts = useMemo(() => {
        // For history, if no search is performed, return empty (as per user request)
        if (statusFilter === 'history' && !searchQuery.trim() && !selectedHistoryUser) return [];
        ...
        const filtered = receipts.filter(r => {
            if (statusFilter === 'pending') {
                ...
                if (r.status === 'pending') return false;
                // History filter by month/year and SPECIFIC USER
                if (selectedHistoryUser && r.student_id !== selectedHistoryUser.id) return false;
        ...
        return filtered;
    }, [receipts, selectedYear, selectedMonth, searchQuery, statusFilter, selectedHistoryUser]);

    const filteredUsersWithReceipts = useMemo(() => {
        if (statusFilter !== 'history' || !searchQuery.trim() || selectedHistoryUser) return [];
        ...
        receipts.forEach(r => {
            ...
            if (name.includes(q) || email.includes(q)) {
                userMap.set(r.student_id, r.profiles);
            }
        });

        return Array.from(userMap.values());
    }, [receipts, searchQuery, statusFilter, selectedHistoryUser]);
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top }]}>
            <View style={styles.header}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <TouchableOpacity
                        ...
                        onPress={() => useAlertStore.getState().showAlert('Info', 'Módulo de reportes en desarrollo')}
                    >
                        <Ionicons name="bar-chart" size={16} color={colors.primary[500]} />
                        <Text style={styles.reportBtnText}>Reportabilidad</Text>
                    </TouchableOpacity>
            ...
            <View style={styles.tabsRow}>
                ...
                >
                    <Text style={[styles.tabText, statusFilter === 'history' && styles.tabTextActive]}>Historial</Text>
                </TouchableOpacity>
            </View>
            ...
            {statusFilter === 'history' && selectedHistoryUser && (
                <TouchableOpacity
                    style={styles.backToUsers}
                    onPress={() => setSelectedHistoryUser(null)}
                >
                    <Ionicons name="arrow-back" size={16} color={colors.primary[500]} />
                    <Text style={styles.backToUsersText}>Volver a resultados de: {searchQuery}</Text>
                </TouchableOpacity>
            )}

            {statusFilter === 'history' && !selectedHistoryUser && searchQuery.trim().length > 0 ? (
                <FlatList
                    data={filteredUsersWithReceipts}
                    keyExtractor={(item) => item.id}
                    contentContainerStyle={styles.list}
                    renderItem={({ item }) => (
                        <TouchableOpacity
                            style={styles.userResultCard}
                            onPress={() => setSelectedHistoryUser(item)}
                        >
                            <View style={styles.userResultAvatar}>
                                <Text style={styles.userResultAvatarText}>{item.full_name?.charAt(0).toUpperCase()}</Text>
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={styles.userResultName}>{item.full_name}</Text>
                    ...
                    ListEmptyComponent={
                        <View style={styles.empty}>
                            ...
                            <Text style={styles.emptyText}>No se encontraron alumnos con historial</Text>
                        </View>
                    }
                />
            ) : (
                    ...
                    ListEmptyComponent={
                        <View style={styles.empty}>
                            ...
                            <Text style={styles.emptyText}>
                                {statusFilter === 'history' && !searchQuery.trim() ? "Busca un alumno para ver su historial" : "No hay comprobantes"}
                            </Text>
                        </View>
                    }
                />
...
const styles = StyleSheet.create({
    ...
    reportBtnText: { fontSize: 12, fontWeight: '600', color: colors.primary[500] },
    ...
    backToUsers: {
        ...
        marginHorizontal: spacing.xl, marginBottom: spacing.md,
        padding: spacing.sm, backgroundColor: colors.primary[500] + '10',
        borderRadius: borderRadius.md,
    },
    backToUsersText: { fontSize: 13, fontWeight: '600', color: colors.primary[500] },
    userResultCard: {
    ...
    userResultAvatarText: { color: colors.primary[500], fontWeight: '700' },
    userResultName: { fontSize: 15, fontWeight: '600', color: colors.text },
    userResultEmail: { fontSize: 12, color: colors.textSecondary },
