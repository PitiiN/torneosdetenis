...
export async function POST(request: NextRequest) {
    try {
        ...
        const { fieldId, userId, dates, startTime, endTime, status, priceTotalCents, verificationNote } = validation.data
        ...
        for (const dateStr of dates) {
            // Generate timestamp with correct offset for Chile (handling DST automatically)
            // Parse local time to UTC date
            const startDt = fromZonedTime(`${dateStr} ${startTime}`, 'America/Santiago')
            const endDt = fromZonedTime(`${dateStr} ${endTime}`, 'America/Santiago')

            // Format back to ISO string with offset (e.g. -03:00 or -04:00)
        ...
        if (createError) {
            ...
            return NextResponse.json(
                {
                    error: `Error al crear las reservas: ${createError.message}`,
                    details: createError
                },
                { status: 500 }
            )
