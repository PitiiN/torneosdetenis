...
- [x] **Phase C: Status Flow Audit**
    - [x] Audit `booking-card.tsx` and admin views for status labels.
    - [x] Update `status-helpers.ts` or components to remove 'RECHAZADA' from user view.
    - [x] Ensure consistent mapping (Internal: RECHAZADA -> User: CANCELADA/Hiden).

- [ ] **Phase D: Production Sanity Check**
    - [x] `npm run build` (Verified exit code 0).
    - [/] Manual Verification of Flows (Pending User Validation).
    - [ ] Deploy.
