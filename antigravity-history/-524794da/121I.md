...
- [/] **Debtors by Field & Detail Modal**
    - [ ] Update `GET /api/admin/debtors` to support `fieldId`.
    - [ ] Update `GET /api/admin/bookings` to support `userId`.
    - [ ] Update `admin/financial/page.tsx` to filter debtors list by field.
    - [ ] Update `BulkPaymentModal.tsx` to fetch and show full history.
    - [ ] Verify debtor filtering and modal details.
