...
export async function POST(request: NextRequest) {
    try {
        ...
        // Update all bookings to PAGADA status
        ...
        if (updateError) {
            console.error('Bulk payment error:', updateError)
            return NextResponse.json(
        ...
        return NextResponse.json({
            ...
            message: `${updatedBookings?.length || 0} reservas marcadas como pagadas`,
        })
    } catch (error) {
