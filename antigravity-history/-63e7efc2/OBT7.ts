...
export async function GET(request: NextRequest) {
    try {
        ...
        const userIds = Array.from(new Set(typedBookings.map(b => b.user_id).filter(Boolean))) as string[]
        console.log(`[FieldUsers] Found ${userIds.length} unique user IDs to fetch profiles for.`)

        const profilesMap = new Map<string, { id: string, full_name: string | null, phone: string | null }>()
        ...
        if (userIds.length > 0) {
            ...
            if (profilesError) {
                console.error('[FieldUsers] Error fetching profiles:', profilesError)
            }

            if (profiles) {
                console.log(`[FieldUsers] Fetched ${profiles.length} profiles.`)
        ...
        for (const booking of typedBookings) {
            ...
            let userFullName = 'Sin Nombre'
            if (profile?.full_name) {
                userFullName = profile.full_name
            } else if (!booking.user_id) {
                userFullName = 'Admin / Cliente Presencial'
            } else {
                userFullName = `Sin Nombre (ID: ${booking.user_id.slice(0, 8)}...)`
            }

            // Email mapping
            const userEmail = (userId === ANONYMOUS_ID ? '-' : null)
