...
export default function HomeScreen() {
    ...
    const seenAvisosDate = useAppStore(s => s.seenAvisosDate);
    ...
    // Badge: count avisos created after last seen date
    const unreadAvisos = seenAvisosDate
        ? announcements.filter(a => a.priority === 'important' && new Date(a.date) > new Date(seenAvisosDate)).length || importantAvisos.length
        : importantAvisos.length;
    ...
    const quickActions = [
        ...
        { title: 'Agenda', emoji: '📅', bg: '#F0FDF4', badge: 0, onPress: () => goToTab('Agenda') },
