...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Clock, User, X, FileText } from 'lucide-react-native';
...
export default function AdminAgendaScreen() {
    ...
    // Generar próximos 7 días para el selector
    const days = useMemo(() => {
    ...
    const timeSlots = useMemo(() => {
        ...
        while (currentHour < endHour || (currentHour === endHour && currentMin < endMin)) {
            ...
            currentMin += 60; // Increment one hour at a time for this simplified grid
            if (currentMin >= 60) {
    ...
    const handleUpdateStatus = async (bookingId: string, newStatus: string) => {
        ...
        if (error) {
            Alert.alert("Error", "No se pudo actualizar la reserva: " + error.message);
        } else {
    ...
    const handleBlockSlot = async (time: string) => {
        if (!selectedField || !selectedDate) {
            Alert.alert("Error", "Debe seleccionar una cancha y una fecha.");
            return;
        }

        setLoading(true);
        const [hour, minute] = time.split(':').map(Number);
        const start_at = new Date(selectedDate);
        start_at.setHours(hour, minute, 0, 0);
        const end_at = new Date(start_at);
        end_at.setHours(hour + 1, minute, 0, 0); // Assuming 1-hour slots
        ...
        const { error } = await supabase.from('bookings').insert({
            ...
            status: 'BLOQUEADA',
            total_price: 0, // Blocked slots have no price
            payment_method: 'N/A',
            verification_note: 'Bloqueado por administrador',
            // user_id can be null for admin blocks or a specific admin user ID
        });

        if (error) {
            Alert.alert("Error", "No se pudo bloquear el horario: " + error.message);
        } else {
                        ...
                        return (
                            <View key={time} style={styles.slotRow}>
                                ...
                                <TouchableOpacity
                                    style={[
                                        styles.slotCard,
                                        booking ? (booking.status === 'PAGADA' ? styles.slotOccupied : booking.status === 'BLOQUEADA' ? styles.slotBlocked : styles.slotPending) : styles.slotAvailable
                                    ]}
                                    onPress={() => {
                                        if (booking) {
                                    ...
                                    {booking ? (
                                        <View style={styles.bookingInfo}>
                                            <View style={styles.bookingHeader}>
                                                <Text style={[styles.bookingName, booking.status === 'PAGADA' ? styles.whiteText : styles.amberText]}>
                                                    {booking.user?.full_name || booking.verification_note || 'Admin/Invitado'}
                                                </Text>
                                                <Badge variant={booking.status === 'PAGADA' ? 'pagada' : 'pendiente'}>
                                                    {booking.status === 'PAGADA' ? 'OK' : 'Prio'}
                                                </Badge>
                                            </View>
                                            <Text style={[styles.bookingDetails, booking.status === 'PAGADA' ? styles.whiteText : styles.amberText]}>
                                                {booking.user?.phone || 'Sin teléfono'}
                                            </Text>
                                        </View>
                                    ) : (
