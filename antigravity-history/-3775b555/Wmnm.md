...
*   **Creación de Torneo**: Un formulario que permite estructurar la cantidad de jugadores, modalidad y formato. Incluye un buscador modal para seleccionar a alumnos registrados en la plataforma. Posee confirmación mediante Pop-up integrado a la paleta (`colors.primary[500]`).
*   **Detalle Dinámico**: Al ingresar a un torneo, muestra las pestañas correspondientes al formato. Permite interactuar con los partidos para cargar los *Scores*. Además, la cabecera expone herramientas para **Eliminar torneo (Basurero)** y **Editar Nombre y Estado (Lápiz)**.
*   **Edición**: Nueva vista dedicada en `/edit/[id]` que rescata vía `query string` el torneo y permite modificar su nombre visible así como gestionar los cambios de Estado ("Borrador", "En Curso" y "Finalizado").
