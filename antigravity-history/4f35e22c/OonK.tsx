...
export default function AdminBlocksPage() {
    ...
    // Grouping Logic
    const groupedBlocks = blocks.reduce<Record<string, BlockGroup>>((acc, block) => {
        const reasonKey = block.reason ? block.reason.trim() : 'Sin Motivo'
        ...
        if (!acc[reasonKey]) {
            acc[reasonKey] = {
                ...
                count: 0,
                fields: [],
                dateRange: { start: block.start_at, end: block.end_at },
                blocks: []
            }
        }

        const group = acc[reasonKey]
        group.count++
        group.blocks.push(block)

        // Update Date Range
        if (block.start_at < group.dateRange.start) group.dateRange.start = block.start_at
        if (block.end_at > group.dateRange.end) group.dateRange.end = block.end_at

        // Add Field Name (unique)
        const fieldName = block.field?.name || 'Cancha eliminada'
    ...
    // Convert to Array and Sort by Recent Activity (e.g. max date)
    const sortedGroups = Object.values(groupedBlocks).sort((a, b) =>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Execute creations sequentially
            let successCount = 0
            let lastError = ''
            for (const bookingPayload of bookingsToCreate) {
                ...
                if (response.ok) {
                    ...
                    lastError = resData.error || resData.details || 'Error desconocido'
                    console.error('Error creating block:', resData)
                }
            }

            if (successCount === 0) {
                setError(lastError || 'Error al crear los bloqueos. Puede que existan conflictos de horario.')
            } else {
                fetchData()
                setShowForm(false)
                setFormData((prev) => ({
    ...
    const handleDelete = async (id: string, type: 'BLOCK' | 'BOOKING' = 'BLOCK') => {
        ...
        try {
            ...
            if (response.ok) {
                fetchData() // This updates 'blocks', causing re-grouping
                // Also update the modal view if open
                if (selectedGroup) {
                    const updatedGroup = { ...selectedGroup }
                    updatedGroup.blocks = updatedGroup.blocks.filter(b => b.id !== id)
                    if (updatedGroup.blocks.length === 0) {
                        setIsDetailOpen(false) // Close if empty
                    } else {
                        setSelectedGroup(updatedGroup) // Update modal content
                    }
                }
            }
        } catch (error) {
    ...
    const handleDeleteGroup = async (group: BlockGroup) => {
        if (!confirm(`¿Estás seguro de eliminar TODOS los ${group.count} bloqueos del grupo "${group.reason}"?`)) return

        setIsDetailOpen(false) // Close modal first
        setLoading(true) // Show loading on main screen

        try {
            // Sequential delete to be safe
            for (const block of group.blocks) {
                await fetch(`/api/admin/blocks/${block.id}?type=${block.type || 'BLOCK'}`, {
                    method: 'DELETE',
                })
            }
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Form Card (Unchanged Logic - Removed for Brevity in this patch, assuming it is preserved by tool or I include it fully? Tool replaces Range so I must include it) */}
            {showForm && (
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Grupos de Bloqueos Activos</CardTitle>
                </CardHeader>
                <CardContent>
                    {loading ? (
                                    ...
                                    >
                                        ...
                                        <TableCell>
                                            <span className="text-sm">
                                                {format(new Date(group.dateRange.start), 'dd/MM')} - {format(new Date(group.dateRange.end), 'dd/MM/yyyy')}
                                            </span>
                                        </TableCell>
            ...
            <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
                <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Detalle de Bloqueos: {selectedGroup?.reason}</DialogTitle>
                        <DialogDescription>
                            Listado completo de bloqueos asociados a este motivo.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="py-4">
                        <div className="flex justify-end mb-4">
                            ...
                            >
                                ...
                                Eliminar Todo el Grupo ({selectedGroup?.count})
                            </Button>
                        </div>
                                    ...
                                    <TableRow key={block.id}>
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col">
                                                <span className="text-xs text-muted-foreground">{block.type === 'BOOKING' ? 'Reserva Bloqueada' : 'Bloqueo'}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <Button
