...
export default function AdminReviewsScreen() {
    ...
    const [stats, setStats] = useState({ total: 0, unread: 0, avg: 0 });
    ...
    const load = useCallback(async () => {
        ...
        if (data) {
            setReviews(data);
            const unread = data.filter((r: any) => !r.is_read).length;
            const avg = data.length > 0
                ? data.reduce((s: number, r: any) => s + (r.rating || 0), 0) / data.length
                : 0;
            setStats({ total: data.length, unread, avg: Math.round(avg * 10) / 10 });
        }
    }, []);
