...
- **Status Revert**: Added "Marcar como Pendiente" button in BulkPaymentModal to revert bookings to 'PENDIENTE_PAGO'.
- **UI Polish**: Implemented fluid transitions in Financial Panel. Switching fields now shows a subtle overlay instead of a full-page reload.
- **Success Modal**: Simplified the "Reservation Confirmed" modal by removing labels and centering bank details in a single column.
...
- **Gestión de Usuarios (Ex-Lista de Morosos)**: 
  - Muestra **todos los usuarios** con reservas en la cancha y **mes seleccionado**.
  - Incluye usuarios anónimos o reservas creadas por administrador.
  - Al hacer click en un usuario, se abre un **Panel de Gestión** filtrado por el mes.
  - **Gestión Total**: 
    - **Pagar**: Marcar reservas seleccionadas como pagadas.
    - **Cancelar**: Cancelar reservas activas.
    - **Pendiente**: Revertir reservas a estado "Pendiente de Pago" (útil para correcciones).
    - **Editar Precio**: Modificar el valor de cualquier reserva directamente desde el panel (icono lápiz).
