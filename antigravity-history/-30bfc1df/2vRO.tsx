...
export function AdminCalendarView() {
    ...
    const fetchWeekBookings = async () => {
        ...
        try {
            ...
            // Ensure we cover the full range including timezones (fetch up to 9 days to be safe against UTC+offset shifts)
            // Fix: Start from previous day to catch early bookings that might be stored as previous day in UTC
            ...
            const bookingsResponse = await fetch(
                `/api/admin/bookings?fieldId=${selectedField.id}&from=${fetchStartDate}T00:00:00&to=${fetchEndDate}T00:00:00`
            )
            ...
            console.log('[AdminCalendar] First booking sample:', weekBookings[0]) // Debug user data
    ...
    const findBookingForSlot = (day: Date, time: string): BookingWithDetails | undefined => {
        ...
        // Construct slot start time in Chile zone
        // 'day' comes from 'weekDays' which are Dates at 00:00 local or UTC? 
        // AdminCalendarView calculates weekDays using addDays(startOfWeek(...))
        // Let's rely on date-fns to handle the comparison properly by creating a specific slot Date.

        // Create a date object for the slot at the specific time in Chile
        const dateStr = format(day, 'yyyy-MM-dd')
        const slotIso = `${dateStr}T${time}:00` // e.g. 2026-02-08T09:30:00

        // We need to compare this against booking start/end times.
        // Bookings are in UTC strings in DB, but we have them as objects.
        // Let's simply check if the slot string matches OR if it falls within.
        // BUT strict string matching works best for exact slots. 
        // For range matching:

        return allBookings.find(b => {
            // Convert Booking UTC to Chile Time
            const bookingStartChile = toZonedTime(b.start_at, 'America/Santiago')
            const bookingEndChile = toZonedTime(b.end_at, 'America/Santiago')

            // Create Slot Date in Chile Time
            const slotDateChile = toZonedTime(day, 'America/Santiago')
            slotDateChile.setHours(slotHour, slotMin, 0, 0)

            // Compare timestamps
            // Slot must be >= start AND < end
            return slotDateChile.getTime() >= bookingStartChile.getTime() &&
                slotDateChile.getTime() < bookingEndChile.getTime()
        })
    }
    ...
    const handleSlotClick = (day: Date, time: string) => {
        ...
        if (status === 'booked' || status === 'pending' || status === 'blocked') {
            ...
            if (booking) {
                ...
                alert(`No se pudieron cargar los detalles de esta reserva (${dateStr} ${time}).\nPosible causa: Diferencia de zona horaria o carga incompleta.\nIntente refrescar.`)
            }
