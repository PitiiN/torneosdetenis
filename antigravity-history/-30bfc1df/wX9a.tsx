...
export function AdminCalendarView() {
    ...
    const renderSlotContent = (status: string, booking?: BookingWithDetails) => {
        if (status === 'booked' || status === 'pending') {
            const userName = booking?.user?.full_name || booking?.user?.email || booking?.verification_note || (booking?.user_id ? 'Usuario' : 'Admin/Invitado')
            return (
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Debug Info */}
            <div className="fixed bottom-4 right-4 bg-black/80 text-white p-4 rounded-lg text-xs z-50 max-w-sm overflow-auto max-h-40">
                <p>Field: {selectedField?.name} ({selectedField?.id})</p>
                <p>Week: {format(currentWeekStart, 'yyyy-MM-dd')}</p>
                <p>Bookings Fetched: {allBookings.length}</p>
                <p>WeekData Keys: {Object.keys(weekData).length}</p>
                {allBookings.length > 0 && (
                    <div className="mt-2 border-t border-white/20 pt-2">
                        <p>First Booking:</p>
                        <p>ID: {allBookings[0].id.slice(0, 8)}...</p>
                        <p>Start: {allBookings[0].start_at}</p>
                        <p>Local: {format(new Date(allBookings[0].start_at), 'yyyy-MM-dd HH:mm')}</p>
                        <p>User: {allBookings[0].user?.full_name || 'No Name'} ({allBookings[0].user_id || 'No ID'})</p>
                    </div>
                )}
            </div>
        </div>
