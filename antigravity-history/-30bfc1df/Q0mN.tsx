...
export function AdminCalendarView() {
    ...
    const fetchWeekBookings = async () => {
        ...
        // 1. Fetch slots (Visibility) - Critical, fail gracefully separately
        try {
        ...
        // 2. Fetch detailed bookings (Editing) - Optional for visibility
        try {
            const fetchStartDate = format(addDays(currentWeekStart, -1), 'yyyy-MM-dd')
            const fetchEndDate = format(addDays(currentWeekStart, 9), 'yyyy-MM-dd')

            // Only fetch if we have a field
            const bookingsResponse = await fetch(
                `/api/admin/bookings?fieldId=${selectedField.id}&from=${fetchStartDate}T00:00:00Z&to=${fetchEndDate}T00:00:00Z`
            )
            const bookingsData = await bookingsResponse.json()

            if (bookingsData.error) {
                console.error('[AdminCalendar] API Error:', bookingsData.error, bookingsData.details)
                // Alert the user about the DB error so they can share it
                const msg = typeof bookingsData.details === 'object'
                    ? JSON.stringify(bookingsData.details, null, 2)
                    : bookingsData.details
                alert(`Error cargando reservas para edición: ${bookingsData.error}\n\nDetalle técnico:\n${msg}`)
                // Don't return, allow function to finish so loading state clears
            } else {
                const weekBookings = (bookingsData.bookings || [])
                    .filter((b: Booking) => b.status !== 'CANCELADA')

                console.log(`[AdminCalendar] Loaded ${weekBookings.length} bookings for week`)
    ...
    const getSlotStatus = (day: Date, time: string): 'available' | 'booked' | 'closed' | 'past' | 'pending' | 'blocked' => {
        ...
        // 1. Check direct bookings (Admin source of truth) - Moved BEFORE past check
    ...
    const findBookingForSlot = (day: Date, time: string): BookingWithDetails | undefined => {
        try {
            const dateStr = format(day, 'yyyy-MM-dd')
            const slotTimeStr = `${dateStr} ${time}`
            const slotDate = fromZonedTime(slotTimeStr, 'America/Santiago')
            const slotTime = slotDate.getTime()

            return allBookings.find(b => {
                // Strategy 1: Range Match (UTC timestamps)
                const startDetails = new Date(b.start_at).getTime()
                const endDetails = new Date(b.end_at).getTime()
                if (slotTime >= startDetails && slotTime < endDetails) return true

                // Strategy 2: Fallback strict string match (Chile Interval)
                // Useful if there are slight ms differences or UTC drifts
                const startChile = toZonedTime(b.start_at, 'America/Santiago')
                const startStr = format(startChile, 'yyyy-MM-dd')
                const timeStr = format(startChile, 'HH:mm')

                if (startStr === dateStr && timeStr === time) return true

                return false
            })
        } catch (e) {
            console.error('Error in findBookingForSlot', e)
            return undefined
        }
    }
    ...
    const handleSlotClick = (day: Date, time: string) => {
        ...
        if (status === 'booked' || status === 'pending' || status === 'blocked') {
            ...
            if (booking) {
                ...
                // Fallback: If status says booked but we can't find details
                console.warn('Booking not found in details for slot', dateStr, time)
                // Debug info
                const debugInfo = `Bookings Loaded: ${allBookings.length}\nSlot: ${dateStr} ${time}`
                alert(`No se pudieron cargar los detalles de esta reserva.\n${debugInfo}\n\nPosible causa: Diferencia de zona horaria o carga incompleta.\nIntente refrescar.`)
            }
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Weekly Grid */}
            <div className="rounded-xl bg-card/50">
                {slotsLoading ? (
                    ...
                    <div className="flex">
                        {/* Fixed time column - OUTSIDE scroll container */}
                        <div className="shrink-0 w-14 sm:w-20 bg-slate-900 z-10 border-r border-border/30">
                            {/* Header */}
                            <div className="h-10 sm:h-12 flex items-center px-2 sm:px-3 border-b border-border/50">
                                <span className="text-[10px] sm:text-xs font-bold text-muted-foreground">Hora</span>
                            </div>
                            {/* Time rows */}
                            {validTimeSlots.map((time: string) => (
                                <div key={time} className="h-9 sm:h-10 flex items-center px-1.5 sm:px-3 border-b border-border/30">
                                    <span className="text-[10px] sm:text-sm font-bold text-muted-foreground whitespace-nowrap">{time}</span>
                                </div>
                            ))}
                        </div>
                        {/* Scrollable days grid */}
                        <div className="flex-1 min-w-0 overflow-x-auto">
                            <div className="flex" style={{ minWidth: '460px' }}>
                                {weekDays.map((day) => (
                                    <div key={day.toISOString()} className="flex-1 min-w-0">
                                        {/* Day header */}
                                        <div className={cn(
                                            'h-10 sm:h-12 flex flex-col items-center justify-center px-1 border-l border-border/30 border-b border-border/50',
                                            isToday(day)
                                                ? 'bg-primary/20 text-primary'
                                                : 'bg-secondary/20'
                                        )}>
                                            <span className="capitalize font-bold text-[10px] sm:text-sm leading-tight">
                                                <span className="hidden sm:inline">{format(day, 'EEEE', { locale: es })}</span>
                                                <span className="sm:hidden">{format(day, 'EEE', { locale: es })}</span>
                                            </span>
                                            <span className="text-[9px] sm:text-xs text-muted-foreground">{format(day, 'dd/MM')}</span>
                                        </div>
                                        {/* Day cells */}
                                        {validTimeSlots.map((time: string) => {
                                            const status = getSlotStatus(day, time)
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className="h-9 sm:h-10 p-0.5 sm:p-1 border-l border-border/30 border-b border-border/30"
                                                >
                                                    <button
                                                        onClick={() => handleSlotClick(day, time)}
                                                        disabled={status === 'closed' || status === 'past'}
                                                        className={cn(
                                                            'w-full h-full rounded-md text-[10px] sm:text-xs font-semibold transition-all duration-150 border relative truncate',
                                                            status === 'available' &&
