...
export function AdminCalendarView() {
    ...
    const findBookingForSlot = (day: Date, time: string): BookingWithDetails | undefined => {
        ...
        // Use timestamps (representing Chile time values) for comparison to avoid minute math errors
        // Note: We use toZonedTime to get a Date where getHours() returns Chile hour
        const slotDateShifted = toZonedTime(day, 'America/Santiago')
        slotDateShifted.setHours(slotHour, slotMin, 0, 0)
        const slotTimeVal = slotDateShifted.getTime()

        return allBookings.find(b => {
            const startDtShifted = toZonedTime(b.start_at, 'America/Santiago')
            const endDtShifted = toZonedTime(b.end_at, 'America/Santiago')

            // Compare purely based on time value ranges
            // This works because we shifted both the slot reference day and the booking times to the same zone (creating "fake" UTC timestamps that represent local time)
            return slotTimeVal >= startDtShifted.getTime() && slotTimeVal < endDtShifted.getTime()
        })
    }
