                ...
                {slotsLoading ? (
                    ...
                    <div className="overflow-x-auto">
                        ...
                        <div className="flex min-w-[520px]">
                            <div className="sticky left-0 z-10 w-14 sm:w-20 shrink-0 bg-slate-900 border-r border-border/30 border-b border-border/50 flex items-center p-2 sm:p-3">
                            ...
                            {weekDays.map((day) => (
                                <div
                                    key={day.toISOString()}
                                    className={cn(
                                        'flex-1 p-1.5 sm:p-3 text-center border-l border-border/30 border-b border-border/50',
                                        isToday(day)
                                            ? 'bg-primary/20 text-primary'
                                            : 'bg-secondary/20'
                                    )}
                                >
                                    <div className="capitalize font-bold text-[10px] sm:text-sm leading-tight">
                                        <span className="hidden sm:inline">{format(day, 'EEEE', { locale: es })}</span>
                                        <span className="sm:hidden">{format(day, 'EEE', { locale: es })}</span>
                                    </div>
                                    <div className="text-[9px] sm:text-xs text-muted-foreground font-normal">
                                        {format(day, 'dd/MM')}
                                    </div>
                                </div>
                            ))}
                        ...
                        {validTimeSlots.map((time: string) => (
                            <div key={time} className="flex min-w-[520px] border-b border-border/30">
                                <div className="sticky left-0 z-10 w-14 sm:w-20 shrink-0 bg-slate-900 border-r border-border/30 flex items-center p-1.5 sm:p-3">
                                    <span className="text-[10px] sm:text-sm font-bold text-muted-foreground">{time}</span>
                                </div>
                                {weekDays.map((day) => {
                                    const status = getSlotStatus(day, time)
                                    const booking = findBookingForSlot(day, time)
                                    ...
                                    return (
                                        <div
                                            ...
                                            className="flex-1 p-1 sm:p-1.5 border-l border-border/30"
                                        >
                                            <button
                                                onClick={() => handleSlotClick(day, time)}
                                                disabled={status === 'closed' || status === 'past'}
                                                className={cn(
                                                    'w-full py-1.5 sm:py-2 px-1 sm:px-2 rounded-md text-[10px] sm:text-xs font-semibold transition-all duration-150 border relative',
                                                    status === 'available' &&
