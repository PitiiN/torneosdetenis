...
export function AdminCalendarView() {
    ...
    const findBookingForSlot = (day: Date, time: string): BookingWithDetails | undefined => {
        const dateStr = format(day, 'yyyy-MM-dd')
        const [slotHour, slotMin] = time.split(':').map(Number)
        const slotDate = toZonedTime(day, 'America/Santiago')
        slotDate.setHours(slotHour, slotMin, 0, 0)

        // Find booking covering this specific time slot
        return allBookings.find(b => {
            // Convert booking times to Chile
            const startDt = toZonedTime(b.start_at, 'America/Santiago')
            const endDt = toZonedTime(b.end_at, 'America/Santiago')

            // Check if date matches (handle overnight later if needed, assume day match for now)
            const bookingDateStr = format(startDt, 'yyyy-MM-dd')
            if (bookingDateStr !== dateStr) return false

            // Check if slot time is >= start and < end
            // Compare timestamps or minutes
            const startMinutes = startDt.getHours() * 60 + startDt.getMinutes()
            const endMinutes = endDt.getHours() * 60 + endDt.getMinutes()
            const currentSlotMinutes = slotHour * 60 + slotMin
