...
## Modal Placement & Component Structure

When implementing interactive grids:
1. **Modal Outside Loops**: Always place modals outside of `.map()` loops. Placing a modal inside the loop replicates the modal DOM for every single slot, causing performance issues and potential Z-index/overlay conflicts.
2. **Centralized Modal State**: Use a singular `selectedSlot` state. When a slot is clicked, update this state, which then triggers the visibility of the single modal instance.
3. **Z-Index Handling**: Ensure the modal container has `z-50` or higher to overlay the grid buttons properly.
4. **Closing Braces/Tags**: Be extremely careful with nesting when using multiple nested `.map()` calls (e.g., `timeSlots.map` nesting `weekDays.map`). Mismatched braces are the most common cause of build failures in complex JSPX layouts.
