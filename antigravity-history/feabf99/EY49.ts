...
export async function POST(request: NextRequest) {
    try {
        ...
        // Force calculation of endAt on server, preserving timezone
        const startDate = parseISO(startAt)
        const endDate = addMinutes(startDate, durationMinutes)
        // Preserve the original timezone offset from input (e.g., -03:00 for Chile)
        const tzMatch = startAt.match(/([+-]\d{2}:\d{2})$/)
        const tzOffset = tzMatch ? tzMatch[1] : '-03:00' // Default to Chile timezone
        const calculatedEndAt = format(endDate, "yyyy-MM-dd'T'HH:mm:ss") + tzOffset
