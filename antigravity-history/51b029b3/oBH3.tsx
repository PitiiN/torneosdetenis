...
import { FixedCalendar as Calendar } from '@/components/ui/fixed-calendar'
                                        ...
                                        <Popover>
                                            <PopoverTrigger asChild>
                                                ...
                                                >
                                                    ...
                                                    {startDate ? (
                                                        format(parseISO(startDate), 'PPP', { locale: es })
                                                    ) : (
                                        ...
                                        <Popover>
                                            <PopoverTrigger asChild>
                                                ...
                                                >
                                                    <CalendarIcon className="mr-2 h-4 w-4" />
                                                    {endDate ? (
                                                        format(parseISO(endDate), 'PPP', { locale: es })
                                                    ) : (
                                                        <span>Seleccionar fecha</span>
                                                    )}
                                                </Button>
                                    ...
                                    >
                                        ...
                                        <SelectContent className="bg-slate-900 border-slate-700 text-white max-h-60">
                                            {(() => {
                                                ...
                                                const [startHour] = schedule.weekday.start.split(':').map(Number)
                                                const [endHour] = schedule.weekday.end.split(':').map(Number)

                                                // Generate slots
                                                // For start times, we go from startHour up to (endHour - 1)
                                                // Example: 19:00 to 22:00 -> Slots: 19:00, 20:00, 21:00
                                                const hoursCount = endHour - startHour

                                                return Array.from({ length: hoursCount }, (_, i) => i + startHour).map(hour => {
                                    ...
                                    >
                                        ...
                                        <SelectContent className="bg-slate-900 border-slate-700 text-white max-h-60">
                                            {(() => {
                                                ...
                                                // For end times, we go from (startHour + 1) up to endHour
                                                // Example: 19:00 to 22:00 -> End times: 20:00, 21:00, 22:00
                                                const hoursCount = endHour - startHour

                                                return Array.from({ length: hoursCount }, (_, i) => i + startHour + 1).map(hour => {
                                                    const time = `${hour.toString().padStart(2, '0')}:${minuteStr}`
                                                    return (
                                                        <SelectItem key={time} value={time}>
                                                            {time}
