...
export default function RegisterScreen() {
    ...
    const handleRegister = async () => {
        if (!fullName || !email || !password) {
            useAlertStore.getState().showAlert('Error', 'Por favor completa todos los campos obligatorios');
            return;
        }
        if (password.length < 6) {
            useAlertStore.getState().showAlert('Error', 'La contraseña debe tener al menos 6 caracteres');
            return;
        }
        if (password !== confirmPassword) {
            useAlertStore.getState().showAlert('Error', 'Las contraseñas no coinciden');
            return;
        }
