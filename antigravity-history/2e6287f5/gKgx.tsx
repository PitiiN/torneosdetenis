...
export default function NeighborhoodMapScreen({ navigation }: any) {
    ...
    const markersJS = mapPins.map(p => {
        ...
        const actionHtml = isActualAdmin
            ? `<br><br><button onclick="window.ReactNativeWebView.postMessage(JSON.stringify({action: \\'edit_pin\\', id: \\'${p.id}\\'}))" style="background:#3B82F6;color:white;border:none;padding:8px 10px;border-radius:6px;width:100%;margin-bottom:8px;">✏️ Editar Pin</button><button onclick="window.ReactNativeWebView.postMessage(JSON.stringify({action: \\'delete_pin\\', id: \\'${p.id}\\'}))" style="background:#EF4444;color:white;border:none;padding:8px 10px;border-radius:6px;width:100%">🗑️ Eliminar Pin</button>`
            : '';
        return `L.marker([${p.lat}, ${p.lng}]).addTo(map).bindPopup('<b>${p.emoji} ${escapedTitle}</b><br>${escapedDesc}<br><small>${typeLabel}</small>${actionHtml}');`;
    }).join('\n');
...
<script>
  ...
  map.on('click', function(e) {
    ...
    const handleAddPin = () => {
        ...
        if (isActualAdmin) {
            if (editingPinId) {
                // Admin: Update existing pin
                updateMapPin(editingPinId, { title: pinTitle.trim(), description: pinDesc.trim() || pinTitle.trim(), category: pinCategory, lat: tappedLat, lng: tappedLng, emoji: pinEmoji });
        ...
        <SafeAreaView style={s.safe}>
            <View style={s.header}>
                ...
                <Text style={s.subtitle}>
                    {isActualAdmin ? `Toca el mapa para seleccionar ubicación • ${mapPins.length} puntos` : "Toca el mapa para agregar un servicio o un punto de interés"}
                </Text>
                {isActualAdmin && (
            ...
            <Modal visible={showAddModal} transparent animationType="fade">
                <TouchableOpacity style={s.modalOverlay} activeOpacity={1} onPress={handleCancelPin}>
                    <View style={s.modalContent} onStartShouldSetResponder={() => true}>
                        ...
                        {!isActualAdmin && <Text style={s.requestNote}>Tu solicitud será revisada por el administrador antes de aparecer en el mapa.</Text>}
                        {tappedLat !== null && (
                        ...
                        <TouchableOpacity style={s.confirmBtn} onPress={handleAddPin}>
                            <Text style={s.confirmText}>{isActualAdmin ? (editingPinId ? '💾 Guardar Cambios' : '✅ Agregar al Mapa') : '📨 Enviar Solicitud'}</Text>
                        </TouchableOpacity>
