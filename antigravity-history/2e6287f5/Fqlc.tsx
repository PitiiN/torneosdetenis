...
export default function NeighborhoodMapScreen({ navigation }: any) {
    ...
    const markersJS = mapPins.map(p => {
        ...
        const actionHtml = isActualAdmin
            ? `<br><br><button onclick="window.ReactNativeWebView.postMessage(JSON.stringify({action: \\'manage_pin\\', id: \\'${p.id}\\'}))" style="background:#EF4444;color:white;border:none;padding:8px 10px;border-radius:6px;width:100%">🗑️ Eliminar Pin</button>`
            : '';
