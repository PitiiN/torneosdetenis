...
- **Unregistered Guest Players**: The system now natively supports rendering and tracking manual player names inside Round Robin groupings and Official Matches. Admins can simply type any text string inside the `PlayerAssignModal` to populate a tournament slot for last-minute guests, without forcing them through Auth registration.
