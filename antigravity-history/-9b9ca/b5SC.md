...
- **`my-classes.tsx`**: Implemented a Supabase real-time subscription. The screen now automatically refreshes whenever the user's enrollments change (e.g., after enrolling from the 'Inicio' screen or cancelling), ensuring the "Next Class" card and the list are always up to date.
