...
- **Navegación Fluida**:
    - Se ha implementado un tema global oscuro (`DarkTheme`) en la raíz de la aplicación (`app/_layout.tsx`). Esto asegura que el fondo del contenedor de navegación coincida con el tema oscuro de la app, eliminando el "flash blanco" que ocurría durante las transiciones entre pantallas.
- **Configuración (Admin)**:
    - Se ha creado la tabla `app_settings` en Supabase (que no existía) y se han configurado políticas RLS para permitir que solo administradores modifiquen precios y valores globales.
