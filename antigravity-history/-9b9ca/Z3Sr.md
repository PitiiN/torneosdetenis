...
- **`payments.tsx`**:
    ...
    - **Refactored `processUpload`**: Initially switched to `blob` via `fetch`, but further refactored to use `FormData`. `FormData` is the most stable and compatible method for uploading binary files in Expo/React Native environments, effectively resolving both the "base64" and "Network request failed" errors.
    - **Deprecation Fix**: Replaced `ImagePicker.MediaTypeOptions` with the modern `['images']` array to eliminate console warnings.
- **Configuración (Admin)**:
    - Se ha creado la tabla `app_settings` en la base de datos de Supabase, la cual no existía a pesar de ser referenciada en el código. Esto soluciona el error "could not find table public.app_settings in schema cache".
    - Se han configurado políticas de seguridad (RLS) para permitir que solo administradores modifiquen los valores globales, mientras que todos los usuarios pueden leerlos.
    - Se han cargado los valores predeterminados para los precios de los packs.
