...
// Generate month options: 1 year back and 1 year forward from now
function getMonthOptions() {
    ...
    // Generate range: 12 months back to 12 months forward
    // Sort descending (newest/future first)
    for (let i = -12; i <= 12; i++) {
        // i negative = future (addMonths), i positive = past (subMonths)
        // We want Future -> Past. 
        // Let's iterate i from -12 (future) to 12 (past)? No, subMonths(now, -1) is future.
    ...
    // Re-sort options by date desc (except "all")
    // The loop above generates: i=-12 (now+12m) ... i=12 (now-12m)
    // Actually subMonths(now, -1) is next month.
    // So loop -12 to 12 generates: Future ... Past. 
    // This is already descending order naturally?
    // -12 => 12 months from now.
    // 0 => Now
    // 12 => 12 months ago.
    // So the array starts with Future and ends with Past. Perfect.
