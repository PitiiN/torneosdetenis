...
export async function GET(request: Request) {
    // 1. Auth Client: To check who is logged in (reads cookies)
    ...
    // 2. Admin Client: To bypass RLS/Policies for data operations
    ...
    for (const booking of newBookings) {
        ...
        // Clean up any existing booking in this slot (Force overwrite)
        await supabase.from('bookings').delete()
            .eq('field_id', bookingData.field_id)
            .eq('start_at', bookingData.start_at)
            .eq('end_at', bookingData.end_at)
