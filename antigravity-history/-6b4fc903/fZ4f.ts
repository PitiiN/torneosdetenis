...
export async function GET(request: Request) {
    ...
    const { data: fields } = await supabase
        .from('fields')
        .select('id, name')
        .in('name', ['Huelén 7', 'Huelén 5', 'Tabancura 6'])

    if (!fields || fields.length === 0) {
        return NextResponse.json({ error: 'No se encontraron las canchas' }, { status: 404 })
    }

    const huelen7 = fields.find(f => f.name === 'Huelén 7')
    const huelen5 = fields.find(f => f.name === 'Huelén 5')
    const tabancura6 = fields.find(f => f.name === 'Tabancura 6')
