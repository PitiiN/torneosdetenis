...
export default function AccessibilityScreen({ navigation }: any) {
    const { fontScale, highContrast, accessibilityMode, ttsEnabled, setFontScale, setHighContrast, setAccessibilityMode, setTtsEnabled } = useAccessibility();
        ...
        <SafeAreaView style={[s.safe, { backgroundColor: bg }]}>
            <ScrollView contentContainerStyle={s.scroll}>
                ...
                <View style={[s.card, { backgroundColor: cardBg }]}>
                    <View style={s.toggleRow}>
                        <View style={s.toggleInfo}>
                            <Text style={[s.cardTitle, { color: textPrimary, fontSize: dynText(16) }]}>🧩 Modo simplificado</Text>
                            <Text style={[s.desc, { color: textSecondary, fontSize: dynText(13) }]}>Reduce elementos visuales para mayor claridad</Text>
                        </View>
                        <Switch value={accessibilityMode} onValueChange={setAccessibilityMode} trackColor={{ true: accent }} />
                    </View>
                </View>
