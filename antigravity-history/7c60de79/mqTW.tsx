...
import { format, differenceInHours, subMonths } from 'date-fns';
...
export default function MyClassesScreen() {
    ...
    useEffect(() => {
        ...
        // Subscribe to real-time changes for this student's enrollments
    ...
    const handleCancel = (enrollment: any) => {
        ...
        if (hoursUntil < 24) {
            useAlertStore.getState().showAlert('No se puede cancelar', 'Solo puedes cancelar hasta 24 horas antes del inicio.');
            return;
        }
    ...
    // Upcoming classes (start_datetime in the future and not cancelled)
    ...
    const past = enrollments.filter((e) => {
        const d = new Date(e.classes.start_datetime);
        return d <= now && d >= subMonths(now, 2);
    });
            ...
            {nextClass ? (
                <View style={styles.heroCard}>
                    ...
                    <View style={styles.heroActions}>
                        {differenceInHours(new Date(nextClass.classes.start_datetime), now) >= 24 && (
    ...
    // listData includes all classes for the active tab (no slice(1) as per user request)
                ...
                renderItem={({ item }) => {
                    ...
                    return (
                        <View style={[styles.classCard, isPast && styles.cardPast]}>
                            ...
                            {!isPast && differenceInHours(new Date(cls.start_datetime), now) >= 24 && (
