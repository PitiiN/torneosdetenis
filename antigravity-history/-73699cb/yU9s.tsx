...
export default function AdminDashboard() {
    ...
    const load = useCallback(async () => {
        ...
        // Chile is UTC-3 (or UTC-4 in summer). We expand the UTC window by 4h on each side
        // so that classes created at any local hour are always captured.
        const dateStr = format(selectedDate, 'yyyy-MM-dd');
        const { data: classesData } = await supabase
            .from('classes_with_availability')
            .select('*')
            .gte('start_datetime', dateStr + 'T00:00:00-04:00')
            .lte('start_datetime', dateStr + 'T23:59:59-03:00')
                                ...
                                { text: 'Abortar', style: 'cancel' },
                                {
                                    ...
                                    onPress: async () => {
                                        try {
                                            ...
                                            const affectedEnrollments = (data as any) || [];
                                            const affectedCount = affectedEnrollments.length;

                                            // Trigger Push Notifications for affected students
                                            if (affectedCount > 0) {
                                                const studentIds = [...new Set(affectedEnrollments.map((e: any) => e.student_id))];
                                                const notificationPromises = studentIds.map(id =>
                                                    notificationsService.createNotification(
                                                        id as string,
                                                        'Clase Suspendida',
                                                        `Tu clase del ${dateStr} ha sido suspendida por condiciones climáticas. Tus créditos han sido devueltos.`,
                                            ...
                                            useAlertStore.getState().showAlert('Éxito', `Día suspendido. Se cancelaron y notificaron ${affectedCount} inscripciones.`);
                                            load();
                                        } catch (err: any) {
    ...
    const handleAutoCancel = async () => {
        const dateStr = format(selectedDate, 'd MMMM', { locale: es });
        useAlertStore.getState().showAlert(
            'Auto-cancelar clases',
            `¿Deseas cancelar automáticamente las clases del ${dateStr} que tengan menos de 3 alumnos inscritos?`,
            [
                    ...
                    onPress: async () => {
                        try {
                            ...
                            const { data: todayClasses } = await supabase
                                .from('classes_with_availability')
                                .select('id, title, start_datetime, enrolled_count')
                                .eq('status', 'scheduled')
                            ...
                            if (!todayClasses || todayClasses.length === 0) {
                                useAlertStore.getState().showAlert('Info', 'No hay clases programadas para este día.');
                                return;
                            }

                            const underMinimum = todayClasses.filter((c: any) => (c.enrolled_count || 0) < 3);
                            if (underMinimum.length === 0) {
                                useAlertStore.getState().showAlert('✅ Todo bien', 'Todas las clases tienen 3 o más alumnos.');
                                return;
                            }

                            let totalNotified = 0;
                            for (const cls of underMinimum) {
                                ...
                                if (enrollments && enrollments.length > 0) {
                                    ...
                                    for (const enr of enrollments) {
                                        await notificationsService.createNotification(
                                            enr.student_id,
                                            'Clase Cancelada',
                                            `La clase "${cls.title}" del ${dateStr} fue cancelada por no alcanzar el mínimo de 3 alumnos. Tu crédito ha sido devuelto.`,
                                            'auto_cancel'
                                        );
                                        totalNotified++;
                                    }
                            ...
                            useAlertStore.getState().showAlert(
                                '✅ Listo',
                                `Se auto-cancelaron ${underMinimum.length} clase(s) y se notificaron ${totalNotified} alumno(s).`
                            );
                            load();
                        } catch (err: any) {
                            useAlertStore.getState().showAlert('Error', err.message);
    ...
    const handleEmptyBlockClick = (hour: number) => {
        useAlertStore.getState().showAlert(
            'Crear Clase',
            `¿Qué tipo de clase deseas crear a las ${String(hour).padStart(2, '0')}:00?`,
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    text: 'Clase Niños 🏗️',
                    onPress: () => useAlertStore.getState().showAlert('En Construcción', 'Esta funcionalidad estará disponible próximamente.')
                },
                {
                    text: 'Clase Adultos',
                    onPress: () => router.push({
            ...
            >
                ...
                <View style={styles.header}>
                    ...
                    <View style={{ flexDirection: 'row', gap: spacing.sm }}>
                        <TouchableOpacity onPress={handleAutoCancel} style={styles.weatherBtn}>
                            <Ionicons name="alert-circle-outline" size={24} color={colors.warning} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={handleWeatherCancellation} style={styles.weatherBtn}>
                            <View style={styles.weatherIconStack}>
                                <Ionicons name="cloud-outline" size={24} color={colors.error} />
                                <Ionicons name="rainy-outline" size={12} color={colors.error} style={styles.rainIcon} />
                            </View>
                        </TouchableOpacity>
                    </View>
                </View>
                                        ...
                                        {hasClass && (
                                            <View style={styles.classInfo}>
                                                <Text style={[styles.hourCategory, isCancelled && { textDecorationLine: 'none' }]} numberOfLines={1}>
                                                    {isCancelled
                                                        ? (cls.auto_cancelled ? 'AUTO-CANCEL' : 'CANCELADA')
                                                        : `${cls.category_name || '?'}/${cls.court2_category_name || '?'}`}
                                                </Text>
                                                {!isCancelled && (
