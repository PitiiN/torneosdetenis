...
export default function AdminDashboard() {
    ...
    const STAT_CARDS = [
        ...
        { icon: 'card', label: 'Ingresos mes actual', value: `$${(stats.revenue / 1000).toFixed(0)}k`, color: colors.accent[500] },
    ];
                            ...
                            {Array.from({ length: block.end - block.start }, (_, i) => {
                                ...
                                return (
                                    <TouchableOpacity
                                        ...
                                        disabled={!hasClass}
                                        onPress={() => hasClass && router.push(`/(admin)/classes/${cls.id}` as any)}
                                        activeOpacity={0.7}
                                    >
                                        ...
                                        {hasClass && (
                                            <>
                                                ...
                                                <View style={styles.spotsRow}>
                                                    <Ionicons name="people" size={10} color={isCancelled ? colors.textTertiary : (hasClass && !isCancelled ? colors.white : colors.textTertiary)} />
                                                    <Text style={[styles.hourStatus, isCancelled && { color: colors.textTertiary }]}>
                                                        {isCancelled ? 'Canc.' : `${cls.max_students - cls.available_spots}/${cls.max_students}`}
                                                    </Text>
                                                </View>
                                            </>
                                        )}
