...
import {
    ...
    RefreshControl, ScrollView, TextInput, ActivityIndicator, Modal, KeyboardAvoidingView, Platform, Linking
} from 'react-native';
...
export default function ProfileScreen() {
    ...
    const [requestTab, setRequestTab] = useState<'new' | 'list'>('new');
    const [requestReason, setRequestReason] = useState('Sugerencia');
    ...
    const loadReplies = async (requestId: string) => {
        const { data } = await supabase.from('student_request_replies')
            .select('*, profiles!student_request_replies_sender_id_fkey(full_name, role)')
    ...
    const submitRequest = async () => {
        if (!requestMessage.trim()) {
            useAlertStore.getState().showAlert('Error', 'Escribe tu mensaje');
            return;
        }
        ...
        if (error) {
            ...
            useAlertStore.getState().showAlert('✅ Éxito', 'Tu solicitud fue enviada correctamente.');
            setRequestMessage('');
            setRequestTab('list');
            loadRequests();
        }
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            >
                ...
                <View style={styles.actionsCard}>
                    ...
                    <TouchableOpacity style={styles.actionRow} onPress={() => setShowRequestModal(true)}>
            ...
            <Modal visible={showSettings} transparent animationType="slide">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <TextInput
                            style={[styles.textInput, { color: colors.textTertiary, backgroundColor: colors.background + '80' }]}
                            value={(profile as any)?.joined_at ? format(new Date((profile as any).joined_at), "d 'de' MMMM, yyyy", { locale: es }) : 'Pendiente por definir'}
                            editable={false}
                        />

                        <TouchableOpacity
                            style={[styles.actionRow, { borderBottomWidth: 0, marginTop: spacing.xl, marginBottom: spacing.lg, backgroundColor: colors.warning + '10', borderRadius: borderRadius.md }]}
            ...
            <Modal visible={showRequestModal} transparent animationType="slide">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <View style={styles.reqTabs}>
                            <TouchableOpacity style={[styles.reqTab, requestTab === 'new' && styles.reqTabActive]} onPress={() => setRequestTab('new')}>
                                <Text style={[styles.reqTabText, requestTab === 'new' && styles.reqTabTextActive]}>Nueva Solicitud</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={[styles.reqTab, requestTab === 'list' && styles.reqTabActive]} onPress={() => setRequestTab('list')}>
                                <Text style={[styles.reqTabText, requestTab === 'list' && styles.reqTabTextActive]}>Mis Solicitudes</Text>
                            </TouchableOpacity>
                        </View>
                        ...
                        {requestTab === 'new' ? (
                            <View>
                                ...
                                <View style={{ flexDirection: 'row', gap: spacing.sm, marginBottom: spacing.lg }}>
                                    {['Sugerencia', 'Reclamo', 'Petición'].map(reason => (
                                        <TouchableOpacity
                                            key={reason}
                                            style={[
                                                styles.reqCategoryBtn,
                                                { flex: 1, paddingVertical: spacing.md, alignItems: 'center', borderRadius: borderRadius.md, borderWidth: 1, borderColor: requestReason === reason ? colors.primary[500] : colors.border },
                                                requestReason === reason && { backgroundColor: colors.primary[500] + '20' }
                                            ]}
                                            onPress={() => setRequestReason(reason)}
                                        >
                                            <Text style={[{ fontSize: 13, color: colors.textSecondary }, requestReason === reason && { color: colors.primary[400], fontWeight: '600' }]}>{reason}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </View>
                                ...
                                />
                                <TouchableOpacity style={[styles.submitBtn, sendingRequest && { opacity: 0.6 }]} onPress={submitRequest} disabled={sendingRequest}>
                                    {sendingRequest ? <ActivityIndicator color={colors.white} /> : <Text style={styles.submitText}>Enviar solicitud</Text>}
                                </TouchableOpacity>
                            </View>
                        ) : (
                            <ScrollView style={{ maxHeight: 400 }}>
                                {requests.length === 0 ? (
                                    <Text style={{ color: colors.textSecondary, textAlign: 'center', marginTop: spacing.xl }}>Ninguna solicitud enviada.</Text>
                                ) : requests.map(req => (
                                    <TouchableOpacity
                                        key={req.id}
                                    ...
                                    >
                                        ...
                                        <View style={[styles.reqStatus, req.status === 'resolved' && styles.reqStatusResolved]}>
                                            <Text style={[styles.reqStatusText, req.status === 'resolved' && styles.reqStatusTextResolved]}>
            ...
            <Modal visible={!!selectedRequest} transparent animationType="slide">
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, { flex: 0.9 }]}>
                        <View style={styles.modalHeader}>
                            <View>
                                <Text style={styles.modalTitle}>{selectedRequest?.reason}</Text>
                                <Text style={{ fontSize: 13, color: colors.textSecondary }}>{selectedRequest?.status === 'pending' ? 'Pendiente de revisión' : 'Resuelta'}</Text>
                            </View>
                        ...
                        <ScrollView style={styles.chatList}>
                            ...
                            <View style={[styles.chatBubble, styles.chatBubbleStudent]}>
                                <Text style={[styles.chatSender, styles.chatSenderRight]}>{profile?.full_name || 'Tú'}</Text>
                                <Text style={[styles.chatText, styles.chatTextStudent]}>{selectedRequest?.message}</Text>
                                <Text style={[styles.chatTime, styles.chatTimeStudent]}>
                                    {selectedRequest?.created_at && format(new Date(selectedRequest.created_at), "d MMM, HH:mm", { locale: es })}
                                </Text>
                            </View>

                            {/* Replies */}
                            {requestReplies.map(reply => {
                                const isAdmin = reply.profiles?.role === 'admin' || reply.profiles?.role === 'coach';
                                return (
                                    <View key={reply.id} style={[styles.chatBubble, isAdmin ? styles.chatBubbleAdmin : styles.chatBubbleStudent]}>
                                        <Text style={[styles.chatSender, isAdmin ? styles.chatSenderLeft : styles.chatSenderRight]}>
                                            {isAdmin ? (reply.profiles?.full_name || 'Academia') : (profile?.full_name || 'Tú')}
                                        </Text>
                                        <Text style={[styles.chatText, isAdmin ? {} : styles.chatTextStudent]}>{reply.message}</Text>
                                        <Text style={[styles.chatTime, isAdmin ? {} : styles.chatTimeStudent]}>
                        ...
                        {selectedRequest?.status === 'pending' && (
                            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
                                <View style={styles.chatInputRow}>
                                    ...
                                    />
                                    <TouchableOpacity style={[styles.chatSendBtn, sendingReply && { opacity: 0.6 }]} onPress={submitReply} disabled={sendingReply}>
...
const styles = StyleSheet.create({
    ...
    reqTabs: { flexDirection: 'row', backgroundColor: colors.background, borderRadius: borderRadius.lg, padding: 4, marginBottom: spacing.lg },
    reqTab: { flex: 1, paddingVertical: spacing.md, alignItems: 'center', borderRadius: borderRadius.md },
    reqTabActive: { backgroundColor: colors.surface },
    reqTabText: { fontSize: 13, fontWeight: '600', color: colors.textTertiary },
    reqTabTextActive: { color: colors.white },
    reqCategoryBtn: { backgroundColor: colors.background, padding: spacing.md, borderRadius: borderRadius.lg, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.lg, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    reqReasonBtn: { backgroundColor: colors.background, padding: spacing.md, borderRadius: borderRadius.lg, borderWidth: 1, borderColor: colors.border, marginBottom: spacing.lg, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    reqReasonText: { color: colors.text, fontSize: 15 },
    reqCard: { backgroundColor: colors.background, borderRadius: borderRadius.lg, padding: spacing.lg, marginBottom: spacing.sm, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    reqReason: { fontSize: 15, fontWeight: '600', color: colors.text, marginBottom: spacing.xs },
    reqMessage: { fontSize: 13, color: colors.textSecondary, width: 220 },
    reqStatus: { paddingHorizontal: spacing.sm, paddingVertical: 4, borderRadius: 12, backgroundColor: colors.primary[500] + '20' },
    reqStatusText: { fontSize: 11, fontWeight: '600', color: colors.primary[500] },
    reqStatusResolved: { backgroundColor: colors.success + '20' },
    reqStatusTextResolved: { color: colors.success },
    ...
    chatBubbleAdmin: { backgroundColor: colors.surface, alignSelf: 'flex-start', borderBottomLeftRadius: 4 },
    chatBubbleStudent: { backgroundColor: colors.primary[500], alignSelf: 'flex-end', borderBottomRightRadius: 4 },
    chatSender: { fontSize: 13, fontWeight: '700', marginBottom: spacing.xs },
    chatSenderLeft: { color: colors.primary[400] },
    chatSenderRight: { color: colors.white, alignSelf: 'flex-end' },
