...
export async function GET(request: NextRequest) {
    try {
        ...
        // Fetch blocks
        let blocksQuery = adminClient
            .from('field_blocks')
            .select('*, field:fields(name)')

        if (fieldId) {
            blocksQuery = blocksQuery.eq('field_id', fieldId)
        }

        // Fetch blocked bookings
        ...
        const normalizedBlocks = (blocksResult.data || []).map((b: any) => ({
            id: b.id,
            field_id: b.field_id,
            start_at: b.start_at,
            end_at: b.end_at,
        ...
        const normalizedBookings = (bookingsResult.data || []).map((b: any) => ({
            ...
            start_at: b.start_at,
            end_at: b.end_at,
            reason: b.verification_note || 'Bloqueo Manual',
            field: b.field,
            type: 'BOOKING',
            created_at: b.created_at
        }))

        const allBlocks = [...normalizedBlocks, ...normalizedBookings].sort((a, b) =>
