...
#### [NEW] [Migration](file:///tbahjhufxmsyldhghhhz)
- Create `public.app_settings` table with columns: `key` (text, PK), `value` (jsonb), `created_at`, `updated_at`.
- Enable RLS on `public.app_settings`.
- Policy: Admins have full access (INSERT, UPDATE, DELETE).
- Policy: All authenticated users have read access (SELECT).
- Pre-populate with `pack_prices` default values.

- Pre-populate with `pack_prices` default values.

---

### [Phase 22: Eliminate Navigation Flash]

#### [MODIFY] [_layout.tsx](file:///c:/Users/Asus/OneDrive%20-%20CORPORACI%C3%93N%20XIX%20JUEGOS%20PANAMERICANOS%20SANTIAGO%202023/Escritorio/PitiN/Antigravity/EscuelaDeTenis/app/_layout.tsx)
- Import `DarkTheme` and `ThemeProvider` from `@react-navigation/native`.
- Define a custom theme that extends `DarkTheme` and uses `colors.background` for the background color.
- Wrap the application (inside `QueryClientProvider`) with `ThemeProvider` using the custom theme.
- This ensures that the underlying navigator defaults to the app's dark background color during any transitions.
...
- Confirm `contentStyle` is set (already is, but ensure consistency).
