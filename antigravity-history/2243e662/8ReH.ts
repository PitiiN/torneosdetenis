...
type AppStore = {
    ...
    addAnnouncementReply: (announcementId: string, message: string, userName: string) => void;
...
export const useAppStore = create<AppStore>()(
    persist(
        (set) => ({
            ...
            addAnnouncementReply: (announcementId, message, userName) => set((state) => ({
                announcements: state.announcements.map(a => a.id === announcementId ? {
                    ...a,
                    replies: [...a.replies, { id: Date.now().toString(), message, userName, date: now() }]
                } : a),
            })),
