...
const TABANCURA_HTML = (whatsapp: string, code: string) => `
    <html>
    <head>
      <style>
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <div class="line"><span class="number">1</span> Deben abrir el candado con el código <strong>${code}</strong></div>
          ...
          <div class="line"><span class="number">4</span> Deben enviar una foto <a href="https://wa.me/${whatsapp}">acá</a> del candado cerrado una vez finalizado el uso</div>
        ...
        <div class="cta">
          <a href="https://wa.me/${whatsapp}">Contáctanos</a>
        </div>
      </div>
    </body>
    </html>
...
const HUELEN_HTML = (whatsapp: string) => `
    <html>
    <head>
      <style>
        body { font-family: 'Segoe UI', sans-serif; background-color: #f4f4f4; margin: 0; padding: 0; }
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="line"><span class="number">4</span> Deben enviar una foto de las luces apagadas (último turno) <a href="https://wa.me/${whatsapp}">acá</a></div>
...
export async function sendBookingConfirmationEmail(to: string, fieldName: string) {
    const whatsapp = '56995158428'; // Clean number for link
    const code = '4224';

    // Normalize field name to check for matches
    const normalizedField = fieldName.toLowerCase().trim();
    let html = '';
    let subject = '';

    if (normalizedField.includes('tabancura')) {
        html = TABANCURA_HTML(whatsapp, code);
        subject = 'Instrucciones Cancha Tabancureños';
    } else if (normalizedField.includes('huelen') || normalizedField.includes('huelén')) {
        html = HUELEN_HTML(whatsapp);
        subject = 'Instrucciones Cancha Huelen';
    } else {
        // Default or no email for other fields?
    ...
    try {
        const data = await resend.emails.send({
            from: 'Arriendo Canchas <noreply@resend.dev>', // Update with verify domain if available, else testing
            to: [to],
            subject: subject,
            html: html,
        });
