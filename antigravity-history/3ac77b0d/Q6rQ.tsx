...
export default function AdminStudentsScreen() {
    ...
    const filteredEnrollments = useMemo(() => {
        return studentEnrollments.filter((enr: any) => {
            ...
            // Extract YYYY-MM prefix from ISO string manually for comparison
            // This is safer than new Date() for just month/year matching in many cases
            const [year, month] = dateStr.split('T')[0].split('-');
            const targetYear = historyMonth.getFullYear().toString();
            const targetMonth = (historyMonth.getMonth() + 1).toString().padStart(2, '0');
