...
export default function AdminStudentsScreen() {
    ...
    const [editStudentCategory, setEditStudentCategory] = useState<string | null>(null);
    ...
    const [requestsLoading, setRequestsLoading] = useState(false); // New state for requests loading

    const loadRequests = async () => {
        setRequestsLoading(true);
        const { data, error } = await supabase
            .from('student_requests')
            .select('*, profiles!student_requests_student_id_fkey(full_name, avatar_url, phone), student_request_replies(id, sender_id, created_at)')
            .eq('status', 'pending')
    ...
    const pendingRequestsCount = useMemo(() => {
        return requests.filter(r => {
            ...
            if (r.student_request_replies && r.student_request_replies.length > 0) {
                const sorted = [...r.student_request_replies].sort((a: any, b: any) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
                if (sorted[0].sender_id !== r.student_id) return false;
            }
            return true;
        }).length;
    }, [requests]);
    ...
    const submitAdminReply = async () => {
        ...
        if (error) {
            ...
            loadRequests(); // Refresh requests to update badge
        }
    };
    ...
    const markAsResolved = async () => {
        ...
        const resolveRequest = async (id: string) => {
            ...
            useAlertStore.getState().showAlert(
                ...
                [
                    { text: 'Cancelar', style: 'cancel', onPress: () => setResolving(false) },
                    {
        ...
        resolveRequest(selectedRequestAdmin.id);
    };
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity
                            ...
                            onPress={async () => {
                                ...
                                setEditStudentCategory(item.student_category || null);
                        ...
                        <View style={styles.modalContent}>
                            <View style={styles.modalHeader}>
                                ...
                                <View style={{ flexDirection: 'row', gap: spacing.md, alignItems: 'center' }}>
                                    {selectedStudent && (editJoinedAt !== (selectedStudent.joined_at || '') || editStudentCategory !== (selectedStudent.student_category || null)) && (
                                        <TouchableOpacity
                                            onPress={async () => {
                                                setSavingDetails(true);
                                                const { error } = await supabase.from('profiles').update({ joined_at: editJoinedAt || null, student_category: editStudentCategory }).eq('id', selectedStudent.id);
                                                setSavingDetails(false);
                                                if (error) {
                                                    console.error(error);
                                                } else {
                                                    setSelectedStudent({ ...selectedStudent, joined_at: editJoinedAt, student_category: editStudentCategory });
                                                    load(); // Refresh list
                                                }
                                            }}
                                    ...
                                    <View style={styles.infoGrid}>
                                        ...
                                        <View style={styles.infoItem}>
                                            ...
                                            <View style={{ flexDirection: 'row', gap: 8, marginTop: 4 }}>
                                                {['Inicial', 'Intermedio', 'Avanzado'].map((cat) => (
                                                    <TouchableOpacity
                                                        key={cat}
                                                        style={[
                                                            { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.background },
                                                            editStudentCategory === cat && { backgroundColor: cat === 'Inicial' ? '#22C55E' : cat === 'Intermedio' ? '#F59E0B' : '#EF4444', borderColor: 'transparent' },
                                                        ]}
                                                        onPress={() => setEditStudentCategory(editStudentCategory === cat ? null : cat)}
                                                    >
                                                        <Text style={[
                                                            { fontSize: 12, fontWeight: '600', color: colors.text },
                                                            editStudentCategory === cat && { color: '#fff' },
                                                        ]}>{cat}</Text>
                                                    </TouchableOpacity>
                                                ))}
                                            </View>
                        ...
                        <View style={[styles.modalContent, { flex: 0.9 }]}>
                            ...
                            <ScrollView style={styles.chatList}>
                                ...
                                <View style={[styles.chatBubble, styles.chatBubbleStudent]}>
                                    <Text style={[styles.chatSender, styles.chatSenderLeft]}>{selectedRequestAdmin?.profiles?.full_name}</Text>
                                ...
                                {requestReplies.map(reply => {
                                    ...
                                    return (
                                        <View key={reply.id} style={[styles.chatBubble, isAdminReply ? styles.chatBubbleAdminReply : styles.chatBubbleStudent]}>
                                            <Text style={[styles.chatSender, isAdminReply ? styles.chatSenderRight : styles.chatSenderLeft]}>
                                                {isAdminReply ? (reply.profiles?.full_name || 'Admin') : (selectedRequestAdmin?.profiles?.full_name || 'Alumno')}
                                            </Text>
