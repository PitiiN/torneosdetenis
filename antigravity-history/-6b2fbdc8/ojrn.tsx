...
export default function BookingsPage() {
    ...
    const filteredBookings = useMemo(() => {
        ...
        return allBookings.filter(booking => {
            ...
            if (statusFilter === 'PENDIENTE_PAGO') {
                // If filter is explicitly 'Pending', maybe exclude 'Hold'? 
                // User asked for both. Usually 'Pending' includes 'Hold'.
                return booking.status === 'PENDIENTE_PAGO'
            }
    ...
    const getStatusBadge = (status: string, createdAt: string) => {
        // Custom Hold Logic for Badge
        if (status === 'PENDIENTE_PAGO') {
            const created = new Date(createdAt)
            const diff = now.getTime() - created.getTime()
            if (diff <= 10 * 60 * 1000) {
                return <Badge className="bg-amber-500 hover:bg-amber-600">Hold</Badge>
            }
            return <Badge variant="outline" className="text-amber-600 border-amber-600">Pendiente</Badge>
        }

        const variants: Record<string, string> = {
            'PAGADA': 'bg-emerald-500 hover:bg-emerald-600',
                        ...
                        <Card key={booking.id} className="overflow-hidden transition-all hover:shadow-md">
                            <CardContent className="p-0">
                                <div className="flex flex-col sm:flex-row">
                                    ...
                                    {/* Right: Actions */}
                                    <div className="p-4 flex flex-row sm:flex-col gap-2 justify-center border-t sm:border-t-0 sm:border-l border-border bg-muted/10">
