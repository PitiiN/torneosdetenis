...
export default function BookingsPage() {
    ...
    // Rebook state
    const [availabilityMap, setAvailabilityMap] = useState<Record<string, boolean | 'loading'>>({})
    ...
    // Check availability for cancelled/expired bookings
    useEffect(() => {
        if (statusFilter !== 'CANCELADA') return

        const checkAvailability = async () => {
            const toCheck = filteredBookings.filter(b =>
                availabilityMap[b.id] === undefined
            )
            ...
            toCheck.forEach(async (booking) => {
                ...
                setAvailabilityMap(prev => ({ ...prev, [booking.id]: 'loading' }))
                ...
                try {
                    ...
                    if (data?.slots?.[dateStr]) {
                        const slot = data.slots[dateStr].find((s: any) => s.startTime === startStr)
                        setAvailabilityMap(prev => ({ ...prev, [booking.id]: slot?.status === 'available' }))
                    } else {
        ...
        checkAvailability()
    }, [filteredBookings, statusFilter, availabilityMap])
    ...
    const getStatusBadge = (booking: Booking) => {
        ...
        if (booking.status === 'PAGADA') {
            return <Badge className="bg-emerald-600 hover:bg-emerald-700 w-full h-full flex items-center justify-center text-sm font-bold uppercase tracking-wider">Pagada</Badge>
        }
            ...
            <div className="space-y-4">
                {filteredBookings.length === 0 ? (
                    ...
                    filteredBookings.map((booking) => {
                        ...
                        const canRebook = availability === true
                        const checkingAvailability = availability === 'loading'
                                            ...
                                            {booking.status === 'PENDIENTE_PAGO' && !expired ? (
                                                ...
                                                <>
                                                    ...
                                                    <Button
                                                        ...
                                                        className={cn(
                                                            "w-full text-xs",
                                                            !canRebook && !checkingAvailability && "cursor-not-allowed opacity-60"
                                                        )}
                                                        disabled={!canRebook || checkingAvailability}
