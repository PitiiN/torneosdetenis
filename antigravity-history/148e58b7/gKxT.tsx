...
export default function AvailabilityPage() {
    ...
    useEffect(() => {
        ...
        fetchFields().catch(() => setLoading(false))
    }, [])
    ...
    const handleSlotClick = (day: Date, time: string) => {
        ...
        // Generate ISO string with correct timezone offset for Chile
        const startDt = fromZonedTime(`${dateStr} ${time}`, 'America/Santiago')
        const endDt = fromZonedTime(`${dateStr} ${endTime}`, 'America/Santiago')

        // Ensure startAt has correct offset
        const startAt = formatInTimeZone(startDt, 'America/Santiago', "yyyy-MM-dd'T'HH:mm:ssXXX")
        const endAt = formatInTimeZone(endDt, 'America/Santiago', "yyyy-MM-dd'T'HH:mm:ssXXX")
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                ...
                <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-center text-xs sm:text-sm font-medium flex-1 sm:flex-none">
            ...
            {/* Legend - Compact Pill Badges */}
            <div className="flex flex-wrap gap-1.5 sm:gap-3">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 sm:px-4 sm:py-2 rounded-full bg-emerald-500/20 border border-emerald-500/50 text-emerald-400 font-medium text-[11px] sm:text-sm">
                    <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-emerald-500 animate-pulse" />
                    Disponible
                </div>
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 sm:px-4 sm:py-2 rounded-full bg-red-500/20 border border-red-500/50 text-red-400 font-medium text-[11px] sm:text-sm">
                    <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-red-500" />
                    Arrendada
                </div>
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 sm:px-4 sm:py-2 rounded-full bg-amber-500/20 border border-amber-500/50 text-amber-400 font-medium text-[11px] sm:text-sm">
                    <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-amber-400 animate-pulse" />
                    Hold
                </div>
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 sm:px-4 sm:py-2 rounded-full bg-purple-500/20 border border-purple-500/50 text-purple-400 font-medium text-[11px] sm:text-sm">
                    <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-purple-500" />
                    Bloqueada
                </div>
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 sm:px-4 sm:py-2 rounded-full bg-slate-500/20 border border-slate-500/50 text-slate-400 font-medium text-[11px] sm:text-sm">
                                            ...
                                            return (
                                                ...
                                                >
                                                    <button
                                                        ...
                                                        className={cn(
                                                            'w-full h-full rounded-md text-[10px] sm:text-xs font-semibold transition-all duration-150 border truncate',
                                                            status === 'available' &&
                                                            ...
                                                            status === 'pending' &&
                                                            'bg-amber-400 text-amber-900 border-amber-500 cursor-not-allowed font-bold',
                                                            status === 'blocked' &&
            ...
            {bookingDetails && (
                <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
                    <Card className="w-full sm:max-w-md rounded-t-2xl sm:rounded-xl max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in duration-300">
