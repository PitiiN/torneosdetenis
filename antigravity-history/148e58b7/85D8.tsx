            ...
            <div className="rounded-xl bg-card/50 overflow-hidden">
                {slotsLoading ? (
                    ...
                    <table className="w-full table-fixed border-collapse">
                        <thead>
                            <tr>
                                <th className="w-10 sm:w-20 p-1 sm:p-2 text-left text-[9px] sm:text-xs font-bold text-muted-foreground bg-slate-900 border-b border-border/50">
                                    Hora
                                </th>
                                {weekDays.map((day) => (
                                    <th
                                        key={day.toISOString()}
                                        className={cn(
                                            'p-0.5 sm:p-2 text-center border-l border-border/30 border-b border-border/50',
                                            isToday(day)
                                                ? 'bg-primary/20 text-primary'
                                                : 'bg-secondary/20'
                                        )}
                                    >
                                        <div className="capitalize font-bold text-[9px] sm:text-sm leading-tight">
                                            <span className="hidden sm:inline">{format(day, 'EEE', { locale: es })}</span>
                                            <span className="sm:hidden">{format(day, 'EEEEE', { locale: es })}</span>
                                        </div>
                                        <div className="text-[8px] sm:text-xs text-muted-foreground font-normal">
                                            {format(day, 'dd/MM')}
                                        </div>
                                    </th>
                                ))}
                        ...
                        <tbody>
                            {validTimeSlots.map((time: string) => (
                                <tr key={time}>
                                    <td className="w-10 sm:w-20 p-1 sm:p-2 text-[9px] sm:text-sm font-bold text-muted-foreground bg-slate-900 border-b border-border/30 whitespace-nowrap">
                                        {time}
                                    </td>
                                    {weekDays.map((day) => {
                                        const status = getSlotStatus(day, time)
                                        ...
                                        return (
                                            <td
                                                ...
                                                className="p-0.5 sm:p-1 border-l border-border/30 border-b border-border/30"
                                            >
                                                <button
                                                    onClick={() => handleSlotClick(day, time)}
                                                    disabled={status !== 'available'}
                                                    className={cn(
                                                        'w-full py-1 sm:py-2 rounded text-[8px] sm:text-xs font-semibold transition-all duration-150 border truncate block',
                                                        status === 'available' &&
                                                        'bg-emerald-500 hover:bg-emerald-600 text-white border-emerald-600 cursor-pointer shadow-sm',
                                                        status === 'booked' &&
