    ...
    try {
        ...
        for (const schedule of typedSchedules) {
            ...
            while (isBefore(addMinutes(slotStart, slotDuration), scheduleEnd) ||
                ...
                type BookingSlot = { id: string; start_at: string; end_at: string; status: string }
                const typedBookings = (bookings || []) as BookingSlot[]
                const blockingBooking = typedBookings.find(b => {
                ...
                type BlockSlot = { id: string; start_at: string; end_at: string }
                const typedBlocks = (blocks || []) as BlockSlot[]
                const blockingBlock = typedBlocks.find(b => {
