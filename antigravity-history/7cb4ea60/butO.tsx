...
export default function SplashScreen() {
    ...
    const handlePlaybackStatusUpdate = (status: AVPlaybackStatus) => {
        if (status.isLoaded) {
            if (status.didJustFinish) {
                setDebugInfo('Video terminado');
                onFinish();
            } else if (status.isPlaying) {
                setDebugInfo('Reproduciendo video...');
            }
        }
        setStatus(status);
    };
    ...
    return (
        <View style={styles.container}>
            <Text style={{ color: 'white', position: 'absolute', top: 50, zIndex: 10, alignSelf: 'center' }}>
                DEBUG: {debugInfo} | User: {user ? 'Sí' : 'No'} | Loading: {isLoading ? 'Sí' : 'No'}
            </Text>
            <Video
                ref={video}
                style={styles.video}
                ...
                onError={(e) => {
                    setDebugInfo('Error de video: ' + JSON.stringify(e));
                    // console.error('Video Error:', e);
                    setTimeout(onFinish, 3000); // 3 segundos para leer el error
                }}
            />
