...
export async function GET(request: NextRequest) {
    try {
        ...
        const { data: bookingsData, error } = await supabase
            ...
            .gte('start_at', `${startDate}T00:00:00`)
            .lte('start_at', `${format(addDays(parseISO(endDate), 1), 'yyyy-MM-dd')}T00:00:00`) // Extend to next day midnight to cover full previous day in any timezone
        ...
        const { data: blocksData } = await supabase
            ...
            .eq('field_id', fieldId)
            .gte('start_at', `${startDate}T00:00:00`)
            .lte('start_at', `${format(addDays(parseISO(endDate), 1), 'yyyy-MM-dd')}T00:00:00`)
