    ...
    try {
        ...
        bookings?.forEach((booking) => {
            ...
            if (booking.status === 'PENDIENTE_PAGO') {
                ...
                const now = Date.now()
                // Allow some buffer or use strict 10 mins. 
                // If created_at is UTC, Date(string) handles it if string has Z. 
                // Supabase returns ISO string with TZ usually.
                if (now - createdTime > 10 * 60 * 1000) {
                    return // Skip (expired), behaves as available
                }
            }

            // Use raw string parsing to avoid timezone shifts
            ...
            const slots: Record<string, { startTime: string; endTime: string; status: 'booked' | 'pending' }[]> = {}
            ...
            bookings?.forEach((booking) => {
                // Use raw string parsing to avoid timezone shifts
                // Assumes DB stores time as "UTC container" for Local Time
                const dateStr = booking.start_at.split('T')[0]
                const startTime = booking.start_at.split('T')[1].substring(0, 5)
                const endTime = booking.end_at.split('T')[1].substring(0, 5)
            ...
            blocks?.forEach((block) => {
                const dateStr = block.start_at.split('T')[0]
                const startTime = block.start_at.split('T')[1].substring(0, 5)
                const endTime = block.end_at.split('T')[1].substring(0, 5)
