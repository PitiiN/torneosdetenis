...
export function BulkPaymentModal({ debtor, fieldId, fieldName, onClose, onSave }: BulkPaymentModalProps) {
    ...
    const handleCancel = async () => {
        ...
        try {
            ...
            // If we cancel, they are no longer "unpaid" for payment purposes, 
            // but they serve as history.
            // Let's reload to update status.
            onSave() // This refreshes parent, but we ideally simply refresh local state?
            // Actually onSave refreshes parent. User might stay in modal?
            // Let's close for simplicity or show success.
            // But we need distinct success message.
            setSuccess(true)
            // We might want to change success message based on action.
            // For now, "Success" state says "Pagos Updated". 
            // We should make success generic or handle it.
        } catch {
            setError('Error de conexión')
        } finally {
            setProcessing(false)
