...
export function BulkPaymentModal({ debtor, fieldId, fieldName, month, onClose, onSave }: BulkPaymentModalProps) {
    ...
    const handleStatusChange = async (newStatus: string) => {
        ...
        try {
            // Use specific endpoints for payment/cancel if they have special logic (emails, etc)
            // But for 'PENDIENTE_PAGO' we use the generic status update
            let endpoint = '/api/admin/bookings/bulk-status'
            if (newStatus === 'CANCELADA') endpoint = '/api/admin/bookings/bulk-cancel'
            // Payment uses bulk-payment which is handled by handleSubmit
            ...
            setSuccessType(newStatus === 'CANCELADA' ? 'cancel' : 'update')
            setSuccess(true)
        } catch {
            setError('Error de conexión')
        } finally {
    ...
    const startEditingPrice = (e: React.MouseEvent, booking: Booking) => {
        e.stopPropagation()
        setEditingPriceId(booking.id)
        setEditingPriceValue((booking.price_total_cents).toString()) // Keep as cents for input? No, typically users edit dollars/CLP.
        // If booking.price_total_cents is 35000, we want 35000 in input.
        // Wait, formatMoney divides by 100?
        // const formatMoney = (cents: number) => ... .format(cents / 100)
        // So yes, stored in cents (e.g. 3500000 for $35.000? No, usually 35000 if using CLP without decimals... wait.)
        // Let's check formatMoney implementation in this file.
        // Line 169: .format(cents / 100).
        // If price is 35000 CLP, then stored as 3500000? 
        // Or if stored as 35000, then formatMoney shows 350? 
        // Let's assume standard "cents" logic: 100 cents = 1 unit.
        // So I should divide by 100 for editing.
        setEditingPriceValue((booking.price_total_cents / 100).toString())
    }
    ...
    const savePrice = async (e: React.MouseEvent, bookingId: string) => {
        e.stopPropagation()
        ...
        try {
            ...
            const response = await fetch('/api/admin/bookings/update-price', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    bookingId,
            ...
            setBookings(prev => prev.map(b =>
                b.id === bookingId
                    ? { ...b, price_total_cents: priceCents }
                    : b
            ))
                    ...
                    {success ? (
                        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-300">
                            ...
                            <h3 className="font-bold text-2xl mb-2 text-white">
                                {successType === 'payment' ? '¡Pagos Actualizados!' :
                                    successType === 'cancel' ? '¡Reservas Canceladas!' : '¡Estado Actualizado!'}
                            </h3>
                            <p className="text-muted-foreground mb-8 text-lg">
                                {successType === 'payment'
                                    ...
                                    : successType === 'cancel'
                                        ...
                                        : `Se actualizaron ${selectedIds.size} reservas exitosamente.`
                                }
                            </p>
