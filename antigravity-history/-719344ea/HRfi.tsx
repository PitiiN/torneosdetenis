...
export default function AdminFinanceScreen({ navigation }: any) {
    ...
    // Search: show matching members only when 3+ chars typed
    ...
    const exportExcel = async (reportType: 'finance' | 'dues') => {
        try {
            ...
            if (reportType === 'finance') {
                ...
                const ws = XLSX.utils.json_to_sheet(duesData.length > 0 ? duesData : [{ 'Info': 'Sin datos' }]);
            ...
            // @ts-ignore - expo-file-system types mismatch but API exists at runtime
