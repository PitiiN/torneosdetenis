...
export default function AdminFinanceScreen({ navigation }: any) {
    ...
    // Item 15a: Allow reverting paid dues to pending
    ...
    const exportExcel = async (reportType: 'finance' | 'dues') => {
        try {
            ...
            // Fix Item 15b: Use 'base64' type directly to avoid Buffer dependency issues
    ...
    // Item 15c: Filter finance entries by selected month
    ...
    return (
        <SafeAreaView style={s.safe}>
            <ScrollView contentContainerStyle={s.scroll}>
                ...
                {tab === 'movimientos' && (<>
                    {/* Month filter for finance entries */}
                    <View style={s.monthSelector}>
                        <TouchableOpacity onPress={() => {
                            if (filterMonth === 0) { setFilterMonth(11); setFilterYear(filterYear - 1); }
                            else setFilterMonth(filterMonth - 1);
                        }} style={s.monthArrow}><Text style={s.monthArrowText}>◀</Text></TouchableOpacity>
                        <Text style={s.monthLabel}>{MONTHS[filterMonth]} {filterYear}</Text>
                        <TouchableOpacity onPress={() => {
...
const s = StyleSheet.create({
    ...
    monthArrow: { padding: 8 }, monthArrowText: { fontSize: 18, color: '#2563EB' },
    monthLabel: { fontSize: 18, fontWeight: 'bold', color: '#1E3A5F', marginHorizontal: 16 },
