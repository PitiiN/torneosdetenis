...
export default function AdminFinanceScreen({ navigation }: any) {
    ...
    const exportExcel = async (reportType: 'finance' | 'dues') => {
        try {
            ...
            // Use Buffer polyfill for reliable base64 conversion
            const wbBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'buffer' });
            const base64 = Buffer.from(wbBuffer).toString('base64');
...
const s = StyleSheet.create({
    ...
    yearDropdown: { backgroundColor: '#FFFFFF', borderRadius: 12, padding: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: '#E2E8F0', elevation: 1 },
