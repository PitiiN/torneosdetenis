...
export default function CreateTournamentScreen() {
    ...
    const generateTournament = async (forceIncomplete: boolean = false) => {
        ...
        try {
            ...
            // 2. Create Tournament Players and get their IDs
            ...
            if (selectedPlayers.length > 0) {
                ...
                tournamentPlayers = pData || [];
            }

            // Create a mapping from profile_id to tournament_player_id
            const playerMap = new Map();
            tournamentPlayers.forEach(tp => playerMap.set(tp.profile_id, tp.id));
            ...
            if (format === 'eliminacion' || format === 'consolacion') {
                const neededPlayers = targetCount;
                let currentRoundGames = Math.ceil(neededPlayers / 2);
                let roundNumber = 1;
                let matchOrder = 1;

                while (currentRoundGames >= 0.5) { // Ensure we handle the last final correctly
                    const actualGames = Math.max(1, Math.floor(currentRoundGames));
                    let roundName = '';
                    if (actualGames === 1) roundName = 'Final';
                    else if (actualGames === 2) roundName = 'Semifinales';
                    else if (actualGames === 4) roundName = 'Cuartos de Final';
                    else roundName = `Ronda ${roundNumber}`;
                    ...
                    for (let i = 0; i < actualGames; i++) {
                        ...
                        if (roundNumber === 1) {
                            const prof1 = selectedPlayers[i * 2];
                            const prof2 = selectedPlayers[i * 2 + 1];
                            p1Id = prof1 ? playerMap.get(prof1.id) : null;
                            p2Id = prof2 ? playerMap.get(prof2.id) : null;
                        }
                    ...
                    if (actualGames === 1 && roundName === 'Final') {
                        ...
                        break; // Stop after final and 3rd place
                    }

                    currentRoundGames = actualGames / 2;
                    roundNumber++;
                }

                // Consolación
                if (format === 'consolacion' && neededPlayers >= 4) {
                    let consolationPlayers = Math.floor(neededPlayers / 2);
                    let cRoundGames = Math.ceil(consolationPlayers / 2);
                    let cRoundNumber = 1;

                    while (cRoundGames >= 1) {
                        let roundName = '';
                        if (cRoundGames === 1) roundName = 'Consolación - Final';
                        else if (cRoundGames === 2) roundName = 'Consolación - Semifinales';
                        else roundName = `Consolación - Ronda ${cRoundNumber}`;

                        for (let j = 0; j < cRoundGames; j++) {
                            matchesToInsert.push({
                                tournament_id: tournamentId,
                                round_name: roundName,
                                round_number: cRoundNumber,
                                match_order: matchOrder++,
                                player1_id: null,
                                player2_id: null
                            });
                        ...
                        if (cRoundGames === 1) {
                            matchesToInsert.push({
                                tournament_id: tournamentId,
                                round_name: 'Consolación - 3er y 4to Puesto',
                                round_number: cRoundNumber,
                                match_order: matchOrder++,
                                player1_id: null,
                                player2_id: null
                        ...
                        cRoundGames = cRoundGames / 2;
                        cRoundNumber++;
                    }
                }
            } else if (format === 'round-robin') {
                let matchOrder = 1;
                // For Round Robin, if incomplete players, we still create the pairings
                const playersForRR = selectedPlayers.length > 0 ? selectedPlayers : Array.from({ length: targetCount }).map(() => null);

                for (let i = 0; i < playersForRR.length; i++) {
                    for (let j = i + 1; j < playersForRR.length; j++) {
                        const prof1 = playersForRR[i];
                        const prof2 = playersForRR[j];
                        ...
                        matchesToInsert.push({
                            ...
                            round_number: 1,
                            match_order: matchOrder++,
                            player1_id: prof1 ? playerMap.get(prof1.id) : null,
                            player2_id: prof2 ? playerMap.get(prof2.id) : null
                        });
                    }
                }
            }
            ...
            Alert.alert('Error', error.message || 'Hubo un problema al crear el torneo.');
        } finally {
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <Modal
                ...
                animationType="fade"
            >
                <View style={[styles.modalOverlay, { backgroundColor: 'rgba(0,0,0,0.7)', flex: 1, justifyContent: 'center', alignItems: 'center' }]}>
                    <View style={[styles.modalContent, { backgroundColor: colors.surface, borderRadius: borderRadius.lg, width: '85%', alignItems: 'center', padding: spacing.xl }]}>
                        <View style={{ width: 64, height: 64, borderRadius: 32, backgroundColor: colors.primary[500] + '20', justifyContent: 'center', alignItems: 'center', marginBottom: spacing.lg }}>
                        ...
                        <TouchableOpacity
                            style={[styles.primaryButton, { width: '100%', backgroundColor: colors.primary[500], paddingVertical: 14, borderRadius: borderRadius.md, alignItems: 'center' }]}
                            onPress={() => {
                                setShowSuccessModal(false);
                                router.replace(`/(admin)/tournaments/${createdTournamentId}` as any);
                            }}
                        >
                            <Text style={{ color: '#ffffff', fontWeight: 'bold' }}>Ver Torneo</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
