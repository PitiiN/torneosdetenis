...
export default function CreateTournamentScreen() {
    ...
    const generateTournament = async () => {
        ...
        try {
            // 1. Create Tournament
            const tournamentName = `Torneo ${category.toUpperCase().replace('-', ' ')}`;
            ...
            // 3. Generate bracket matches (Placeholder logic for elimination)
            // Fully generating the bracket matches logic will be complex, 
            // but we can scaffold empty matches depending on the format later.
            // For now, let's create it and navigate to the list or detail view.

            Alert.alert('Éxito', 'Torneo generado correctamente.');
            router.back();
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} bounces={false}>
                ...
                <View style={styles.formSection}>
                    ...
                    {renderPicker('Categoría', 'podium', category, setCategory, [
                        { label: 'Categoría A', value: 'cat-a' },
                        { label: 'Categoría B', value: 'cat-b' },
                        { label: 'Categoría C', value: 'cat-c' },
                    ...
                    {renderPicker('Tipo de Sets', 'time', setType, setSetType, [
                        ...
                        { label: 'Set corto a 4 juegos', value: 'short4' },
            ...
            <View style={[styles.bottomBar, { paddingBottom: insets.bottom || spacing.md }]}>
                <TouchableOpacity
                    style={[
                        styles.submitBtn,
                        (selectedPlayers.length !== neededPlayers || isSubmitting) && styles.submitBtnDisabled
                    ]}
                    onPress={generateTournament}
                    disabled={selectedPlayers.length !== neededPlayers || isSubmitting}
                >
                    {isSubmitting ? (
