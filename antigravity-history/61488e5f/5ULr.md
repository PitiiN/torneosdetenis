...
- [ ] User runs raw SQL snippet in Supabase to fix `categories` and `rr_group_members`.
- [ ] Refactor `AdminScreen.tsx` to display a clean list of Tournaments (Master View).
- [ ] Render Detail View when a Tournament is selected.
- [ ] Update `finishTournamentWizard` to insert empty `rr_group_members` for Round Robin visualizations.
- [ ] Update `PlayerAssignModal` to support assigning players directly to empty `rr_group_members` blocks.
