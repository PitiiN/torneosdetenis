...
export default function PaymentsScreen() {
    ...
    const [allowance, setAllowance] = useState<any>({ total_paid_classes: 0, used_classes: 0, remaining_classes: 0 });
    ...
    const load = useCallback(async () => {
        ...
        const [allowanceRes, receiptsRes] = await Promise.all([
            supabase.rpc('get_student_class_allowance', {
    ...
    useEffect(() => {
        ...
        // Subscribe to changes that affect allowance: receipts, memberships, and enrollments
        const channel = supabase
            .channel(`payments-sync-${profile.id}`)
            .on('postgres_changes', { event: '*', schema: 'public', table: 'payment_receipts', filter: `student_id=eq.${profile.id}` }, () => load())
            .on('postgres_changes', { event: '*', schema: 'public', table: 'product_memberships', filter: `student_id=eq.${profile.id}` }, () => load())
            .on('postgres_changes', { event: '*', schema: 'public', table: 'enrollments', filter: `student_id=eq.${profile.id}` }, () => load())
            .subscribe();
    ...
    const processUpload = async (asset: any) => {
        ...
        try {
            ...
            // Use FormData for better compatibility with Android/Expo and Supabase
    ...
    const pickAndUpload = async () => {
        ...
        if (!asset || !asset.uri) {
            useAlertStore.getState().showAlert('Error', 'No se pudo obtener la imagen seleccionada.');
            return;
        }
