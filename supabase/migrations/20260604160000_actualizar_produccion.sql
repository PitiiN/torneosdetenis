...
ALTER TABLE public.tournament_registration_requests DROP CONSTRAINT tournament_registration_requests_proof_path_chk;
DROP VIEW public.organizations_public;

CREATE EXTENSION pg_graphql WITH SCHEMA graphql;
CREATE FUNCTION public.build_ranking(p_tournament_ids uuid[])
...
$function$;
CREATE FUNCTION public.delete_organization_cascade(p_organization_id uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
$function$;
CREATE FUNCTION public.get_player_achievements(p_player_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
AS $function$
...
END;
$function$;
CREATE FUNCTION public.get_player_match_stats(p_match public.matches, p_player_id uuid)
...
END;
$function$;
CREATE FUNCTION public.get_player_profile_stats(p_player_id uuid, p_org_id uuid, p_level text, p_modality text, p_selected_year integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
AS $function$
...
END;
$function$;
CREATE FUNCTION public.get_score_text(p_score jsonb)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
...
END;
$function$;
CREATE FUNCTION public.get_tournament_placements(p_tournament_id uuid)
 RETURNS TABLE(player_id uuid, place integer, points integer)
 LANGUAGE plpgsql
 STABLE
AS $function$
DECLARE
...
END;
$function$;
CREATE FUNCTION public.parse_set_score(set_score text)
...
END;
$function$;
CREATE FUNCTION public.resolve_match_winner_side(p_match public.matches)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
...
END;
$function$;
CREATE FUNCTION public.set_ranking_manual_adjustment_updated_at()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
begin
...
CREATE INDEX idx_matches_players ON public.matches (player_a_id, player_b_id, player_a2_id, player_b2_id);
ALTER TABLE public.notifications ADD CONSTRAINT notifications_type_check CHECK (type = ANY (ARRAY['match_start'::text, 'result'::text, 'round_advance'::text, 'info'::text, 'registration'::text, 'ranking_position_updated'::text, 'ranking_category_updated'::text, 'ranking_new_number_one'::text, 'registration_request'::text, 'new_tournament_published'::text, 'next_match_defined'::text, 'match_schedule_updated'::text, 'registration_approved'::text, 'tournament_finished'::text, 'match_reminder_24h'::text]));
ALTER TABLE public.organizations ADD COLUMN bases_url text;
ALTER TABLE public.profiles ADD CONSTRAINT profiles_role_check CHECK (role IS NULL OR (role = ANY (ARRAY['player'::text, 'organizer'::text, 'admin'::text, 'referee'::text, 'super_admin'::text])));
CREATE TABLE public.ranking_manual_adjustments (id uuid DEFAULT gen_random_uuid() NOT NULL, organization_id uuid NOT NULL, level text NOT NULL, modality text DEFAULT 'singles'::text NOT NULL, player_id uuid NOT NULL, points integer DEFAULT 0 NOT NULL, created_by uuid NOT NULL, updated_by uuid NOT NULL, created_at timestamp with time zone DEFAULT now() NOT NULL, updated_at timestamp with time zone DEFAULT now() NOT NULL);
ALTER TABLE public.ranking_manual_adjustments ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.ranking_manual_adjustments ADD CONSTRAINT ranking_manual_adjustments_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id) ON DELETE RESTRICT;
ALTER TABLE public.ranking_manual_adjustments ADD CONSTRAINT ranking_manual_adjustments_modality_check CHECK (modality = ANY (ARRAY['singles'::text, 'dobles'::text]));
ALTER TABLE public.ranking_manual_adjustments ADD CONSTRAINT ranking_manual_adjustments_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.organizations(id) ON DELETE CASCADE;
ALTER TABLE public.ranking_manual_adjustments ADD CONSTRAINT ranking_manual_adjustments_pkey PRIMARY KEY (id);
ALTER TABLE public.ranking_manual_adjustments ADD CONSTRAINT ranking_manual_adjustments_player_id_fkey FOREIGN KEY (player_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
ALTER TABLE public.ranking_manual_adjustments ADD CONSTRAINT ranking_manual_adjustments_unique_context UNIQUE (organization_id, level, modality, player_id);
ALTER TABLE public.ranking_manual_adjustments ADD CONSTRAINT ranking_manual_adjustments_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.profiles(id) ON DELETE RESTRICT;
CREATE INDEX ranking_manual_adjustments_context_idx ON public.ranking_manual_adjustments (organization_id, level, modality);
CREATE TRIGGER ranking_manual_adjustments_set_updated_at BEFORE UPDATE ON public.ranking_manual_adjustments FOR EACH ROW EXECUTE FUNCTION public.set_ranking_manual_adjustment_updated_at();
CREATE POLICY ranking_manual_adjustments_delete_org_admin ON public.ranking_manual_adjustments FOR DELETE TO authenticated USING (public.is_org_admin(organization_id));
CREATE POLICY ranking_manual_adjustments_insert_org_admin ON public.ranking_manual_adjustments FOR INSERT TO authenticated WITH CHECK ((public.is_org_admin(organization_id) AND (created_by = auth.uid()) AND (updated_by = auth.uid())));
CREATE POLICY ranking_manual_adjustments_select_authenticated ON public.ranking_manual_adjustments FOR SELECT TO authenticated USING (true);
CREATE POLICY ranking_manual_adjustments_update_org_admin ON public.ranking_manual_adjustments FOR UPDATE TO authenticated USING (public.is_org_admin(organization_id)) WITH CHECK ((public.is_org_admin(organization_id) AND (updated_by = auth.uid())));
CREATE INDEX idx_registrations_tournament_player ON public.registrations (tournament_id, player_id);
ALTER TABLE public.tournament_registration_requests ADD CONSTRAINT tournament_registration_requests_proof_path_chk CHECK (length(proof_path) >= 10 AND length(proof_path) <= 500 AND proof_path !~~ '%..%'::text AND proof_path ~* '^payment-proofs/[0-9a-f-]+/[0-9a-f-]+/[0-9a-f-]+/[A-Za-z0-9._-]+[.](jpg|jpeg|png|webp|heic|heif)$'::text) NOT VALID;
ALTER TABLE public.tournament_registration_requests VALIDATE CONSTRAINT tournament_registration_requests_proof_path_chk;
ALTER TABLE public.tournaments ADD COLUMN players_can_submit_scores boolean DEFAULT false NOT NULL;
CREATE POLICY matches_update_player_autocarga_policy ON public.matches FOR UPDATE TO authenticated USING (((EXISTS ( SELECT 1
   FROM public.tournaments t
  WHERE ((t.id = matches.tournament_id) AND (t.players_can_submit_scores = true)))) AND (EXISTS ( SELECT 1
   FROM public.registrations r
  WHERE ((r.tournament_id = matches.tournament_id) AND (r.player_id = auth.uid())))))) WITH CHECK (((EXISTS ( SELECT 1
   FROM public.tournaments t
  WHERE ((t.id = matches.tournament_id) AND (t.players_can_submit_scores = true)))) AND (EXISTS ( SELECT 1
   FROM public.registrations r
  WHERE ((r.tournament_id = matches.tournament_id) AND (r.player_id = auth.uid()))))));
CREATE INDEX idx_tournaments_parent_org ON public.tournaments (parent_tournament_id, organization_id) WHERE parent_tournament_id IS NOT NULL;
CREATE INDEX idx_tournaments_org_level ON public.tournaments (organization_id, level);
CREATE VIEW public.organizations_public WITH (security_barrier=true) AS SELECT id,
    name,
    slug,
    logo_url,
    created_at,
