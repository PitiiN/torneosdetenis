-- Migration: Robust BYE exclusion from player statistics and rankings
-- Date: 2026-07-04

-- 1. Helper function: url_decode
...
-- 2. Helper function: get_rr_pairings
CREATE OR REPLACE FUNCTION public.get_rr_pairings(p_num_players INT)
RETURNS INT[]
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
    ...
    v_temp INT;
BEGIN
    v_players := ARRAY(SELECT i FROM generate_series(0, p_num_players - 1) i);
    IF p_num_players % 2 <> 0 THEN
        v_players := array_append(v_players, -1);
    END IF;
    ...
    FOR r IN 0..v_rounds - 1 LOOP
        FOR m IN 0..v_matches_per_round - 1 LOOP
            v_p1 := v_players[m + 1];
            v_p2 := v_players[v_n - m];
            IF v_p1 <> -1 AND v_p2 <> -1 THEN
                v_pairings := array_cat(v_pairings, ARRAY[v_p1, v_p2]);
            END IF;
        END LOOP;
        
        v_temp := v_players[v_n];
        v_players := array_cat(
            ARRAY[v_players[1], v_temp],
            v_players[2:v_n - 1]
        );
    END LOOP;
...
DECLARE
    ...
    v_p2 INT;
BEGIN
    -- Check if any player ID is explicitly 'BYE'
    IF p_match.player_a_id::TEXT = 'BYE' OR p_match.player_b_id::TEXT = 'BYE' OR
       p_match.player_a2_id::TEXT = 'BYE' OR p_match.player_b2_id::TEXT = 'BYE' THEN
        RETURN TRUE;
    END IF;

    -- Get tournament description
    SELECT description INTO v_desc FROM tournaments WHERE id = p_match.tournament_id;
    IF v_desc IS NULL OR v_desc = '' THEN
        RETURN FALSE;
    END IF;

    -- Extract MANUAL_ASSIGNMENTS
    v_encoded := substring(v_desc from '\[MANUAL_ASSIGNMENTS:([^\]]+)\]');
    IF v_encoded IS NULL OR v_encoded = '' THEN
        RETURN FALSE;
    END IF;

    v_decoded := url_decode(v_encoded);
    IF v_decoded IS NULL OR v_decoded = '' THEN
        RETURN FALSE;
    END IF;
    ...
    -- Check matchSlots (bracket/elimination)
    v_match_id := p_match.id::TEXT;
    IF p_match.group_name IS NULL OR p_match.group_name = '' THEN
        IF v_json ? 'matchSlots' AND (v_json -> 'matchSlots') ? v_match_id THEN
            v_slots := v_json -> 'matchSlots' -> v_match_id;
            IF (v_slots -> 'p1' ->> 'name') ilike 'BYE' OR
               (v_slots -> 'p2' ->> 'name') ilike 'BYE' OR
               (v_slots -> 'p3' ->> 'name') ilike 'BYE' OR
               (v_slots -> 'p4' ->> 'name') ilike 'BYE' OR
               (v_slots -> 'player_a' ->> 'name') ilike 'BYE' OR
               (v_slots -> 'player_b' ->> 'name') ilike 'BYE' THEN
                RETURN TRUE;
            END IF;
        END IF;
        RETURN FALSE;
    ...
    -- Check rrSlots (round robin)
    v_group_name := p_match.group_name;
    IF v_json ? 'rrSlots' AND (v_json -> 'rrSlots') ? v_group_name THEN
        v_slots := v_json -> 'rrSlots' -> v_group_name;
        
        FOR v_key, v_val IN SELECT * FROM jsonb_each(v_slots) LOOP
            IF (v_val ->> 'name') ilike 'BYE' THEN
                v_bye_slot := (regexp_replace(v_key, '^slot_(\d+)_.*$', '\1'))::INT;
            END IF;
            IF v_key like 'slot_%' THEN
                v_temp := (regexp_replace(v_key, '^slot_(\d+)_.*$', '\1'))::INT;
                IF v_temp + 1 > v_slot_count THEN
                    v_slot_count := v_temp + 1;
                END IF;
            END IF;
        END LOOP;

        IF v_bye_slot <> -1 AND v_slot_count > 0 AND p_match.match_order IS NOT NULL THEN
            v_pairings := get_rr_pairings(v_slot_count);
            v_pairing_idx := (p_match.match_order - 1) * 2;
            IF v_pairing_idx + 2 <= array_length(v_pairings, 1) THEN
                v_p1 := v_pairings[v_pairing_idx + 1];
                v_p2 := v_pairings[v_pairing_idx + 2];
                IF v_p1 = v_bye_slot OR v_p2 = v_bye_slot THEN
                    RETURN TRUE;
                END IF;
            END IF;
        END IF;
...
-- 5. Redefine get_player_profile_stats
